...
In Core2, the upper bound for the system-time is applied per-source; the default upper bound for VT is applied at the query level - this is because each data source has their own system-time timeline, but VT is assumed to be a single timeline, consistent across data sources.
...
The basis map can also contain a `:current-time` option, which applies to any entities that don't specify other valid-time constraints (TODO: see #285).
