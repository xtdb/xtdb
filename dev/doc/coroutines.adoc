...
At the database level this is a single `job.cancelAndJoin()` over the whole tree (every term, the compactor and the source-log subscription run under that one `job`); at the role level it is a scoped cancel of just that role's Job (used when a role changes).
...
* `close()` MUST stay non-suspend, so a role change that closes a term mid-flight completes within the joined coroutine frame rather than suspending.
...
Because freeing rides on the explicit teardown (role change or db close) rather than on Job completion, a unit that *crashes internally with no following role change* holds its resources until the next lifecycle event — a role change (which cancel-then-closes the old term) or database close.
This is by design.
