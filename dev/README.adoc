...
** Individual tests: identify the module they live in, and (for Clojure tests) replace dashes with underscores.
   
   Most Clojure tests live in the root module (under `/src/test/clojure`) so that they're available by default in the root REPL.
   e.g. `./gradlew :test --tests 'xtdb.api_test*'`

   You can also run with a wildcarded keyword - e.g. `./gradlew :test --tests '*expression*'`.
