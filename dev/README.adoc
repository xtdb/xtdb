...
* link:CODING.adoc[Coding practices in XTDB2] — the conventions code is held to
...
You will need JDKs 21 and 25 installed.
We compile for 21, which is still our minimum, but test on 25 — what the Docker images ship.
...
== Running on another JDK

`Test`, `JavaExec` and `clojureRepl` tasks run on JDK 25 by default; `-PtestJavaVersion=21` runs them on 21 instead — linking the custom JRE from it, or launching it directly under `-PfullJdk`.
Compilation targets 21 either way, so this is how to check that nothing has crept in that 21 can't run.
Gradle detects JDKs in the usual install locations, and additionally from `JAVA_HOME_21_X64` / `JAVA_HOME_25_X64` (see `gradle.properties`), which is how CI's `setup-java` hands them over.
...
To record a Java Flight Recorder profile instead, pass `-Pjfr`.
The recording runs for the life of the JVM and is dumped to `~/Snapshots/xtdb-<timestamp>.jfr` when it exits.
...
The conventions code in this repo is held to — general principles, comments, errors, Clojure style, writing tests, and ship/show/ask — live in link:CODING.adoc[].
