...
;; TODO: Should test algorithm exists but couldn't get this to work,
;; might be return code related?
;;   (^int gcry_md_test_algo [^int algo]))

;; (def ^GCrypt lgc (.load (LibraryLoader/create GCrypt) "gcrypt"))
;; (def ^jnr.ffi.Runtime lgc-rt (jnr.ffi.Runtime/getRuntime lgc))
;; (def ^:const sha1-algo (.gcry_md_map_name lgc "SHA1"))
;; (def ^:const sha1-dlen (.gcry_md_get_algo_dlen lgc sha1-algo))


;; TODO: Would need to be thread local, or passed in by caller as most
;; likely there will be a destination in mind.
;; (def ^org.agrona.DirectBuffer digest (mem/allocate-buffer sha1-dlen))

;; TODO: Returning bytes here
;; (defn sha1 ^DirectBuffer [^DirectBuffer bs]
;;   (let [d-p (Pointer/wrap lgc-rt (.addressOffset digest))
;;         b-p (Pointer/wrap lgc-rt (.addressOffset (mem/->off-heap bs)))]
;;     (.gcry_md_hash_buffer lgc
;;                           sha1-algo
;;                           d-p
;;                           b-p
;;                           (mem/capacity bs))
