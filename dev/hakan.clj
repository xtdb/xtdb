...
(def ^:const block-bytes (quot 512 Byte/SIZE))

(defn sha1 [^org.agrona.DirectBuffer from ^org.agrona.MutableDirectBuffer to]
  (let [extra (long (mod (.capacity from) block-bytes))
        blocks (quot (.capacity from) block-bytes)
        limit (* block-bytes blocks)
        pad (doto (org.agrona.concurrent.UnsafeBuffer. (byte-array block-bytes))
              (.putBytes 0 from limit extra)
              (.putByte extra (unchecked-byte 0x80))
              (.putLong (- block-bytes Long/BYTES) (* Byte/SIZE (.capacity from)) java.nio.ByteOrder/BIG_ENDIAN))
        to (org.agrona.concurrent.UnsafeBuffer. to 0 20)
        w (int-array 80)
        is (int-array [(unchecked-int 0x67452301)
                       (unchecked-int 0xEFCDAB89)
                       (unchecked-int 0x98BADCFE)
                       (unchecked-int 0x10325476)
                       (unchecked-int 0xC3D2E1F0)])]
    (loop [current-block (int 0)]
      (if (= (inc blocks) current-block)
        (doto to
          (.putInt (* 0 Integer/BYTES) (aget is 0) java.nio.ByteOrder/BIG_ENDIAN)
          (.putInt (* 1 Integer/BYTES) (aget is 1) java.nio.ByteOrder/BIG_ENDIAN)
          (.putInt (* 2 Integer/BYTES) (aget is 2) java.nio.ByteOrder/BIG_ENDIAN)
          (.putInt (* 3 Integer/BYTES) (aget is 3) java.nio.ByteOrder/BIG_ENDIAN)
          (.putInt (* 4 Integer/BYTES) (aget is 4) java.nio.ByteOrder/BIG_ENDIAN))
        (let [^org.agrona.DirectBuffer block (if (= blocks current-block)
                                               pad
                                               (org.agrona.concurrent.UnsafeBuffer. from  (* block-bytes current-block) block-bytes))]
          (loop [n (int 0)
                 a (aget is 0)
                 b (aget is 1)
                 c (aget is 2)
            ...
            (if (= 80 n)
              ...
              (do
                (if (< n 16)
                  (aset w n (.getInt block (* n Integer/BYTES) java.nio.ByteOrder/BIG_ENDIAN))
                  (aset w n (Integer/rotateLeft
                             (bit-xor
                              (aget w (- n 3))
                              (aget w (- n 8))
                              (aget w (- n 14))
                ...
                (let [f+k (cond
                            ...
                            (+ (bit-or (bit-and b c)
                                       (bit-and (bit-not b) d))
                               (unchecked-int 0x5A827999))

                            (<= n 39)
                            (+ (bit-xor b c d)
                               (unchecked-int 0x6ED9EBA1))

                            (<= n 59)
                            (+ (bit-or (bit-and b c)
                                       (bit-and b d)
                                       (bit-and c d))
                               (unchecked-int 0x8F1BBCDC))

                            (<= n 79)
                            (+ (bit-xor b c d)
                               (unchecked-int 0xCA62C1D6)))]

                  (recur (unchecked-inc-int n)
                         (+ (Integer/rotateLeft a 5)
                            (unchecked-int f+k)
                            e
                            (aget w n))
                         a
                         (Integer/rotateLeft b 30)
                         c
                         d)))))

          (recur (unchecked-inc-int current-block)))))))
