...
;; Rocks JNR Spike in an attempt to gain control over copying across
;; the JNI boundary.
...
;;   (^void rocksdb_writebatch_put [^jnr.ffi.Pointer b ^{jnr.ffi.annotations.Pinned true :tag bytes} key ^{jnr.ffi.types.size_t true :tag long} klen ^{jnr.ffi.annotations.Pinned true  :tag bytes} val ^{jnr.ffi.types.size_t true :tag long} vlen])
;;   (^void rocksdb_writebatch_delete [^jnr.ffi.Pointer b ^{jnr.ffi.annotations.Pinned true :tag bytes} key ^{jnr.ffi.types.size_t true :tag long} klen])
