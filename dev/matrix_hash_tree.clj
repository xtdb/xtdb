...
(comment
  ...
  (matrix-hash-tree/load-rdf-into-lmdb lm-lubm matrix-hash-tree/lubm-triples-resource)

  ;; Try LUBM query 9:
  (with-open [snapshot (kv/new-snapshot lm-lubm)]
    (= (matrix-hash-tree/query
        (matrix-hash-tree/lmdb->graph snapshot)
...
(comment
  ...
  (matrix-hash-tree/load-rdf-into-lmdb lm-watdiv matrix-hash-tree/watdiv-triples-resource)
  ...
  (defn run-watdiv-reference
    ...
    ([lm resource skip?]
     (with-open [snapshot (crux.kv/new-snapshot lm)]
       (let [wg (matrix-hash-tree/lmdb->graph snapshot)
             times (mapv
                    (fn [{:keys [idx query crux-results]}]
                      (let [start (System/currentTimeMillis)]
                        (try
                          (let [result (count (matrix-hash-tree/query wg (crux.sparql/sparql->datalog query)))]
