(ns matrix-mmap
  ...
  (:import [java.util ArrayList Date HashMap HashSet IdentityHashMap Map]
           ...
           crux.ByteUtils
           [java.io DataInputStream DataOutput DataOutputStream FileInputStream FileOutputStream RandomAccessFile]
...
(defn- new-snapshot-matrix [snapshot ^ByteBuffer mmap ^Date business-time ^Date transaction-time idx-id p seek-b ^MutableDirectBuffer content-hash-b bitmap-buffer-cache]
            ...
            (if-not row-id+k
              ...
              (let [row-id (.row-id ^RowIdAndKey row-id+k)]
                (if (not= no-matches-for-row row-id)
                  (let [offset (.getLong ^DirectBuffer (kv/value i) 0 ByteOrder/BIG_ENDIAN)
                        row (lru/compute-if-absent bitmap-buffer-cache
                                                   offset
                                                   (fn [_]
                                                     (swap! cache-miss inc)
                                                     (let [b (ImmutableRoaringBitmap. (.position mmap offset))]
                                                       b
                                                       #_(if (<= (.getCardinality b) bitmap-in-memory-threshold)
...
;; changes.
(def ^:private ^Map file->bitmap-buffer-cache (HashMap.))
(def ^:private mmap-cache (lru/new-cache 32))

(defn lmdb->graph
  ([snapshot mmap-file]
   (let [now (cio/next-monotonic-date)]
     (lmdb->graph snapshot mmap-file now now)))
  ([snapshot mmap-file business-time transaction-time]
   (let [seek-b (ExpandableDirectByteBuffer.)
         ...
         raf (RandomAccessFile. (io/file mmap-file) "r")
         mmap-length (.length raf)
         [mmap
          bitmap-buffer-cache]
         (locking mmap-cache
           [(lru/compute-if-absent
             mmap-cache
             [(str mmap-file) mmap-length]
             (fn [_]
               (with-open [channel (.getChannel raf)]
                 (.remove file->bitmap-buffer-cache mmap-file)
                 (prn :mapping (str mmap-file) mmap-length)
                 (.map channel FileChannel$MapMode/READ_ONLY 0 mmap-length))))
            (.computeIfAbsent file->bitmap-buffer-cache
                              mmap-file
                              (reify Function
                                (apply [_ _]
                                  (lru/new-cache (* 2 1024 1024)))))])]
      ...
      :p->so (reify ILookup
               (valAt [this k]
                 (.computeIfAbsent matrix-cache
                                   ...
                                   (reify Function
                                     (apply [_ _]
                                       (new-snapshot-matrix snapshot mmap business-time transaction-time p->so-idx-id k seek-b content-hash-b bitmap-buffer-cache)))))
      ...
      :p->os (reify ILookup
               (valAt [this k]
                 (.computeIfAbsent matrix-cache
                                   ...
                                   (reify Function
                                     (apply [_ _]
                                       (new-snapshot-matrix snapshot mmap business-time transaction-time p->os-idx-id k seek-b content-hash-b bitmap-buffer-cache)))))
...
(def ^:private ^:const page-alignment-round-mask 0xfff)

(defn kv->bitmaps-file ^java.io.File [kv]
  (io/file (str (kv/db-dir kv) ".bitmaps")))

(defn load-rdf-into-lmdb
  ([kv resource]
   (load-rdf-into-lmdb kv resource 100000))
  ([kv resource chunk-size]
   (let [mmap-file (kv->bitmaps-file kv)]
     (with-open [in (io/input-stream (io/resource resource))
                 raf (RandomAccessFile. mmap-file "rw")]
       (when (zero? (.length raf))
         (doto raf
           (.setLength (inc page-alignment-round-mask))
           (.writeLong (inc alignment-round-mask))
           (.seek 0)))
       (with-open [out (.getChannel raf)]
         (.seek raf (.readLong raf))
         (let [now (cio/next-monotonic-date)
               business-time now
           ...
           (doseq [chunk (->> (rdf/ntriples-seq in)
             ...
             (with-open [snapshot (kv/new-snapshot kv)]
               (let [g (lmdb->graph snapshot mmap-file business-time transaction-time)
                     row-cache (HashMap.)
                     ...
                     bitmap-kvs (with-open [i (kv/new-iterator snapshot)]
                                  (->> (for [[_ v] kvs
                                         ...
                                         (do (.put bitmap->hash v k)
                                             (when-not (and (not (.containsKey hash-key->bitmap c-k))
                                                            (within-prefix? c-k (kv/seek i c-k)))
                                               (let [c-k (mem/copy-buffer c-k)]
                                                 (.put hash-key->bitmap c-k v)
                                                 [c-k (mem/copy-buffer v-serialized)]))))
                                       (remove nil?)
                                       (into {})))
                     hash->position (HashMap.)
                     _ (doseq [[^DirectBuffer k ^DirectBuffer v] bitmap-kvs
                               :let [position (.position out)
                                     new-aligned-offset (bit-and (+ position (.capacity v) alignment-round-mask)
                                                                 (bit-not alignment-round-mask))]]
                         (when (> new-aligned-offset (.length raf))
                           (let [new-size (bit-and (+ (* 2 position) page-alignment-round-mask)
                                                   (bit-not page-alignment-round-mask))]
                             (prn :resize new-size)
...
(doto ^MutableDirectBuffer (mem/allocate-buffer Long/BYTES)
                         ...
                         (when-not (= (+ position (.capacity v))
                                      new-aligned-offset)
                           (.position out new-aligned-offset)))
                     hash-position-kvs (->> (for [[^DirectBuffer k v] hash->position]
                                              [(mem/copy-buffer
                                                (doto content-hash-b
                                                  (.putBytes Integer/BYTES k 0 (.capacity k))))
                                               v])
                                            (into {}))
                     kvs (with-open [i (kv/new-iterator snapshot)]
                           (->> (for [[^DirectBuffer k v] kvs]
                                  [k (if (instance? MutableRoaringBitmap v)
                                       (or (.get hash->position (.get bitmap->hash v))
                                           (let [k ^DirectBuffer (.get bitmap->hash v)
                                                 c-k (doto content-hash-b
                                                       (.putBytes Integer/BYTES k 0 (.capacity k)))]
                                             (if (within-prefix? c-k (kv/seek i c-k))
                                               (mem/copy-buffer (kv/value i))
                                               (throw (IllegalStateException. (str "Excepted position for: " (mem/buffer->hex c-k)))))))
                                       v)])
                                (into hash-position-kvs)))
                     position (.getFilePointer raf)]
                 (doto raf
                   (.seek 0)
                   (.writeLong position)
                   (->> (.getFD) (.sync))
                   (.seek position))
                 (prn :new-position position)
                 (kv/store kv kvs)
                 (prn :written-triples (count chunk))
                 (prn :writen-kvs (count kvs)))))))))))
...
(comment
  ...
  (def lm-lubm (kv/open (kv/new-kv-store "crux.kv.lmdb.LMDBKv")
                        {:db-dir "dev-storage/matrix-lmdb-lubm-mmap"}))
  ...
  (with-open [snapshot (kv/new-snapshot lm-lubm)]
    (= (matrix/query
        (matrix/lmdb->graph snapshot (matrix/kv->bitmaps-file lm-lubm))
...
(comment
  ...
  (def lm-watdiv (kv/open (kv/new-kv-store "crux.kv.lmdb.LMDBKv")
                          {:db-dir "dev-storage/matrix-lmdb-watdiv-mmap"}))
  ...
  (defn run-watdiv-reference
    ...
    ([lm resource skip?]
     (with-open [snapshot (crux.kv/new-snapshot lm)]
       (let [wg (matrix/lmdb->graph snapshot (matrix/kv->bitmaps-file lm))
             times (mapv
