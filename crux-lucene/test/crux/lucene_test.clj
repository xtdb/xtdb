...
(defn submit+await-tx
  ...
  ([node tx-ops]
   (let [tx (fix/submit+await-tx node tx-ops)]
     (l/await-tx node tx {:timeout (Duration/ofSeconds 1)})
     tx)))
...
(t/deftest test-works-with-tx-fns
  (submit+await-tx [[:crux.tx/put {:crux.db/id :put-ivan
                                   :crux.db.fn/body '(fn [ctx]
...
(t/deftest test-ensure-lucene-store-keeps-last-tx
  (let [latest-tx (fn [] (db/latest-completed-tx (:crux.lucene/lucene-store @(:!system *api*))))]
    (t/is (not (latest-tx)))
...
(t/deftest test-handle-incorrect-match-1456
  ...
  (t/is (= (:crux.tx/tx-id (db/latest-completed-tx (:crux/index-store @(:!system *api*))))
           (db/latest-completed-tx (:crux.lucene/lucene-store @(:!system *api*))))))
