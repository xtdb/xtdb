...
(t/deftest test-multi-field-lucene-queries
  (lt/submit+await-tx [[:crux.tx/put {:crux.db/id :ivan
                                      :firstname "Fred"
                                      :surname "Smith"}]])
...
(t/deftest test-namespaced-keywords
  (lt/submit+await-tx [[:crux.tx/put {:crux.db/id :ivan :person/surname "Smith"}]])
...
(t/deftest test-evict
  (let [in-crux? (fn []
    ...
    (lt/submit+await-tx [[:crux.tx/put {:crux.db/id :ivan :name "Smith"}]])

    (assert (in-crux?))
    (assert (in-lucene-store?))

    (lt/submit+await-tx [[:crux.tx/evict :ivan]])
