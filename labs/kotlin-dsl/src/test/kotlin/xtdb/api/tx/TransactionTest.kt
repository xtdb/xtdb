...
class TransactionTest {
    ...
    inner class BaseJavaXtdb {
        ...
        @Test
        fun `can put a document`() = xtdb.run {
        ...
        @Test
        fun `can put a document at time`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can put a document with end valid time`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can delete a document`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can delete a document at valid time`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can delete a document with end valid time`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can evict a document`() = xtdb.run {
            val document = aDocument()
        ...
        @Test
        fun `can match a document`() = xtdb.run {
        ...
        @Test
        fun `can match a document at a valid time`() = xtdb.run {
            val matchDoc = aDocument()
            val document = aDocument()
        ...
        @Test
        fun `can match against no document`() = xtdb.run {
            val matchDoc = aDocument()
            val document = aDocument()
        ...
        @Test
        fun `can match against a no document at valid time`() = xtdb.run {
            val matchDoc = aDocument()
            val document = aDocument()
    ...
    inner class TransactionDslGroupings {
        @Test
        fun `valid time groupings`() = xtdb.run {
        ...
        @Test
        fun `between times groupings`() = xtdb.run {
            val document1 = aDocument()
            val document2 = aDocument()
            val document3 = aDocument()
    ...
    object XtdbPerson : XtdbDocumentSerde<Person> {
    ...
    @Nested
    inner class XtdbDocumentSerdeTests {
        private fun aPersonWithDocument(): Pair<Person, XtdbDocument> {
        ...
        @Test
        fun `can put a person at time`() = xtdb.run {
            val (person, document) = aPersonWithDocument()
            submitTx {
                put(person by XtdbPerson from dates[1])
            }.await()
        ...
        fun `can put a document with end valid time`() = xtdb.run {
            ...
            submitTx{
                put(person by XtdbPerson from dates[1] until dates[2])
            }.await()
