...
(def ^:const ^:private indexed-content-hashes-index-id 8)

(def ^:const ^:private cav-index-id 9)

(def ^:const ^:private standalone-tx-events-index-id 10)
(def ^:const ^:private standalone-doc-index 11)
  ...
  (^org.agrona.MutableDirectBuffer
   ...
   (let [^MutableDirectBuffer b (or b (mem/allocate-buffer (+ index-id-size id-size (.capacity v) (.capacity entity))))]
     (-> (doto b
         ...
         (mem/limit-buffer (+ index-id-size id-size (.capacity v) (.capacity entity)))))))
...
(defn encode-ae-key-to
  (^org.agrona.MutableDirectBuffer [b attr]
   (encode-ae-key-to b attr empty-buffer))
  (^org.agrona.MutableDirectBuffer [^MutableDirectBuffer b ^DirectBuffer attr ^DirectBuffer entity]
   (assert (= id-size (.capacity attr)) (mem/buffer->hex attr))
   (assert (or (= id-size (.capacity entity))
               (zero? (.capacity entity))) (mem/buffer->hex entity))
   (let [^MutableDirectBuffer b (or b (mem/allocate-buffer (+ index-id-size id-size (.capacity entity))))]
     (mem/limit-buffer
      (doto b
        (.putByte 0 ae-index-id)
        (.putBytes index-id-size attr 0 id-size)
        (.putBytes (+ index-id-size id-size) entity 0 (.capacity entity)))
      (+ index-id-size id-size (.capacity entity))))))

(defn decode-ae-key->e-from [^DirectBuffer k]
  (assert (= (+ index-id-size id-size id-size) (long (.capacity k))) (mem/buffer->hex k))
  (assert (= ae-index-id (.getByte k 0)))
  (Id. (mem/slice-buffer k (+ index-id-size id-size) id-size) 0))
...
(defn encode-cav-key-to
  (^org.agrona.DirectBuffer [^org.agrona.MutableDirectBuffer b, ^DirectBuffer content-hash]
   (encode-cav-key-to b content-hash empty-buffer empty-buffer))
...
(defn decode-cav-key->v-from [^DirectBuffer k]
  (assert (<= (+ index-id-size id-size id-size) (.capacity k)) (mem/buffer->hex k))
  (assert (= cav-index-id (.getByte k 0)))
  (let [value-size (- (.capacity k) index-id-size id-size id-size)]
    (mem/slice-buffer k (+ index-id-size id-size id-size) value-size)))

;;;; standalone indices

(defn encode-standalone-tx-event-key-to ^org.agrona.MutableDirectBuffer [^MutableDirectBuffer b, {:crux.tx/keys [tx-id tx-time]}]
  (let [^MutableDirectBuffer b (or b (mem/allocate-buffer (+ index-id-size Long/BYTES Long/BYTES)))]
    (doto b
      (.putByte 0 standalone-tx-events-index-id)
      (.putLong index-id-size tx-id ByteOrder/BIG_ENDIAN)
      (.putLong (+ index-id-size Long/BYTES)
                (date->reverse-time-ms (or tx-time (Date.)))
                ByteOrder/BIG_ENDIAN))))

(defn standalone-tx-event-key? [^DirectBuffer k]
  (= standalone-tx-events-index-id (.getByte k 0)))

(defn decode-standalone-tx-event-key-from [^DirectBuffer k]
  (assert (= (+ index-id-size Long/BYTES Long/BYTES) (.capacity k)) (mem/buffer->hex k))
  (assert (standalone-tx-event-key? k))
...
(defn encode-standalone-doc-key-to ^org.agrona.MutableDirectBuffer [^MutableDirectBuffer b ^DirectBuffer content-hash]
  (assert (= id-size (.capacity content-hash)) (mem/buffer->hex content-hash))
  (let [^MutableDirectBuffer b (or b (mem/allocate-buffer (+ index-id-size id-size)))]
    (mem/limit-buffer
     (doto b
       (.putByte 0 standalone-doc-index)
       (.putBytes index-id-size (mem/as-buffer content-hash) 0 (.capacity content-hash)))
     (+ index-id-size id-size))))
