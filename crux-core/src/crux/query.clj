...
(defn- bound-result-for-var ^crux.query.BoundResult [index-store document-store var->bindings join-keys join-results var]
    ...
    (if (.value? binding)
      ...
      (when-let [^EntityTx entity-tx (get join-results (.e-var binding))]
        (let [value-buffer (get join-keys (.result-index binding))
              ...
              doc (get (db/fetch-docs document-store #{content-hash}) content-hash)
  ...
  (for [[{:keys [pred return] :as clause} idx-id] pred-clause+idx-ids
    ...
    (do (validate-existing-vars var->bindings clause pred-vars)
        {:join-depth pred-join-depth
         :constraint-fn
         (fn pred-constraint [index-store document-store db idx-id->idx join-keys join-results]
           (let [[pred-fn & args] (for [arg (cons pred-fn args)]
                                    (if (logic-var? arg)
                                      (.value (bound-result-for-var index-store document-store var->bindings join-keys join-results arg))
                                      arg))]
...
(defn- or-single-e-var-triple-fast-path [index-store {:keys [query-engine entity-as-of-idx] :as db} {:keys [e a v] :as clause} args]
  (let [{:keys [document-store]} query-engine
  ...
  (for [[clause idx-id [{:keys [free-vars bound-vars]} :as or-branches]] or-clause+idx-id+or-branches
    ...
    (do (validate-existing-vars var->bindings clause bound-vars)
        {:join-depth or-join-depth
         :constraint-fn
         (fn or-constraint [index-store document-store db idx-id->idx join-keys join-results]
           (let [args (when (seq bound-vars)
                        [(->> (for [var bound-vars]
                                (.value (bound-result-for-var index-store document-store var->bindings join-keys join-results var)))
                              (zipmap bound-vars))])
  ...
  (for [{:keys [op args]
    ...
    (do (validate-existing-vars var->bindings clause unification-vars)
        {:join-depth unification-join-depth
         :constraint-fn
         (fn unification-constraint [index-store document-store db idx-id->idx join-keys join-results]
  ...
  (for [not-clause not-clauses
    ...
    (do (validate-existing-vars var->bindings not-clause not-vars)
        {:join-depth not-join-depth
         :constraint-fn
         (fn not-constraint [index-store document-store db idx-id->idx join-keys join-results]
           (with-open [index-store ^Closeable (open-index-store db)]
             (let [db (assoc db :index-store index-store)
...
(defn- constrain-join-result-by-constraints [index-store db idx-id->idx depth->constraints join-keys join-results]
  (let [document-store (get-in db [:query-engine :document-store])]
    (reduce
     (fn [results constraint]
       (when results
         (constraint index-store document-store db idx-id->idx join-keys results)))
     join-results
    ...
    (vec (for [^BoundResult bound-result bound-result-tuple
           ...
           (if-let [entity-tx (and (c/valid-id? value)
             ...
             (let [content-hash (.content-hash ^EntityTx entity-tx)]
               (get (db/fetch-docs document-store #{content-hash}) content-hash))
             value)))))
...
(defn query
  ([{:keys [valid-time transact-time query-engine] :as db} ^ConformedQuery conformed-q]
   (let [{:keys [^ExecutorService query-executor document-store indexer options]} query-engine
  ...
  ([{:keys [valid-time transact-time query-engine] :as db} index-store ^ConformedQuery conformed-q]
   (let [{:keys [indexer document-store options]} query-engine
         q (.q-normalized conformed-q)
     ...
     (let [entity-as-of-idx (cond-> (db/new-entity-as-of-index index-store valid-time transact-time)
       ...
       (cond->> (for [[join-keys join-results] (idx/layered-idx->seq n-ary-join)
                      :let [bound-result-tuple (for [var find]
                                                 (bound-result-for-var index-store document-store var->bindings join-keys join-results var))]]
                  (if full-results?
...
(defn entity [{:keys [query-engine] :as db} index-store eid]
  (let [{:keys [document-store]} query-engine
    ...
    (when-let [content-hash (some-> (:crux.db/content-hash entity-tx) (c/new-id))]
      (-> (get (db/fetch-docs document-store #{content-hash}) content-hash)
    ...
    (let [index-store (open-index-store this)
      ...
      (cio/->cursor #(cio/try-close index-store)
                    ...
                    (assoc (c/entity-tx->edn entity-tx)
                           :crux.db/doc (get (db/fetch-docs document-store #{content-hash}) content-hash))))))
...
(def query-engine
  {:start-fn (fn [{:crux.node/keys [indexer document-store bus]} {::keys [query-pool-size query-cache-size conform-cache-size] :as args}]
                  ...
                  {:indexer indexer
                   ...
                   :options (update args ::entity-cache? (fn [x]
                                                           (if (bound? #'*with-entity-cache?*)
                                                             *with-entity-cache?*
                                                             x)))})))
   :deps [:crux.node/indexer :crux.node/document-store :crux.node/bus]
