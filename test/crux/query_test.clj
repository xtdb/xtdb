...
(t/deftest test-basic-query
  (let [[ivan petr] (f/transact-people! db [{:name "Ivan" :last-name "Ivanov"}
    ...
    (t/testing "Can query by single field"
      (t/is (= #{[(:crux.kv/id ivan)]} (q/q db {:find ['e]
                                                :where [['e :name "Ivan"]]})))
      (t/is (= #{[(:crux.kv/id petr)]} (q/q db {:find ['e]
                                                :where [['e :name "Petr"]]}))))
...
(t/deftest test-basic-query-at-t
  (let [[malcolm] (f/transact-people! db [{:name "Malcolm" :last-name "Sparks"}]
    ...
    (cr/-put db [[(:crux.kv/id malcolm) :name "Malcolma"]] (c/to-date (time/date-time 1986 10 24)))
    (let [q {:find ['e]
             :where [['e :name "Malcolma"]
                     ['e :last-name "Sparks"]]}]
      (t/is (= #{} (q/q db q (c/to-date (time/date-time 1986 10 23)))))
      (t/is (= #{[(:crux.kv/id malcolm)]} (q/q db q))))))
