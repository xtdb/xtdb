...
(t/deftest test-can-correct-ranges-in-the-past
  (let [object-store (idx/->KvObjectStore f/*kv*)
        tx-log (tx/create-kv-tx-log f/*kv* object-store)
    ...
    (with-open [tx-log-context (db/new-tx-log-context tx-log)]
      (let [log (db/tx-log tx-log tx-log-context nil)]
        ...
        (t/is (= [{:crux.tx/tx-id tx1-id
                   ...
                   :crux.tx/tx-ops [[:crux.tx/put (c/new-id :ivan) (c/new-id tx1-ivan) tx1-valid-time nil]]}
                  {:crux.tx/tx-id tx2-id
                   :crux.tx/tx-time tx2-tx-time
                   :crux.tx/tx-ops [[:crux.tx/put (c/new-id :ivan) (c/new-id tx2-ivan) tx2-valid-time nil]
                                    [:crux.tx/put (c/new-id :petr) (c/new-id tx2-petr) tx2-valid-time nil]]}]
                 log))))))
