...
(defn- gen-avg-daily-driving-session
  "AvgDailyDrivingSession - avg session length per driver per day (no random params)
   Uses LAG to detect driving session start/stop transitions (velocity > 5 = driving)."
  [_state]
  {:query-type :avg-daily-driving-session
   :sql "WITH readings_with_prev AS (
           SELECT t.name, r.velocity, r._valid_from,
                  DATE_TRUNC(DAY, r._valid_from) AS day_bucket,
                  LAG(r.velocity) OVER (PARTITION BY t._id ORDER BY r._valid_from) AS prev_velocity
           FROM trucks t
           JOIN readings FOR ALL VALID_TIME AS r ON r._id = t._id
           WHERE t.name IS NOT NULL
         ),
         session_starts AS (
           SELECT name, day_bucket, _valid_from AS session_start
           FROM readings_with_prev
           WHERE velocity > 5 AND (prev_velocity <= 5 OR prev_velocity IS NULL)
         ),
         sessions_per_day AS (
           SELECT name, day_bucket, COUNT(*) AS session_count
           FROM session_starts
           GROUP BY name, day_bucket
         )
         SELECT name, AVG(session_count) AS avg_daily_sessions
...
(defn- generate-queries
  ...
  (let [all-results (for [gen query-generators
        ...
        skipped-types (clojure.set/difference all-query-types generated-types)]
