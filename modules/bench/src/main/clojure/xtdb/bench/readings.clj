...
(defn docs
  ([n] (docs n 10000))
  ([n devices]
   (->> (tu/->instants :minute 5)
        (partition 2 1)
        (mapcat (fn [[start end]]
                  (for [i (range 0 devices 1000)]
                    (into [:put-docs {:into :readings :valid-from start :valid-to end}]
                          (for [j (range i (min devices (+ i 100)))]
                            {:xt/id j :value (random-float -100 100)})))))
        (take (* n devices)))))

(def max-valid-time-q "SELECT max(_valid_to) AS max_valid_time FROM readings FOR ALL VALID_TIME
                       WHERE _id = 0")
...
(defn ->query-stage
  ...
  ([interval offset]
   {:t :do
    :stage (if offset
             (keyword (str "query-offset-" (name offset) "-interval-" (name interval)))
             (keyword (str "recent-query-" (name interval))))
   ...
   (concat (if load-phase
             [{:t :do
               ...
               :tasks [{:t :call :f (fn [{:keys [sut]}]
                                      (doseq [batch (partition-all 1000 (docs size devices))]
...
(comment
  ...
  (def max-valid-time (time (-> (xt/q node max-valid-time-q {:basis {:at-tx latest-completed-tx}}) first :max-valid-time)))

  (time (check-query node (subtract-period max-valid-time :week) max-valid-time
                     {:basis {:at-tx latest-completed-tx
                              :current-time (:system-time latest-completed-tx)}})))
