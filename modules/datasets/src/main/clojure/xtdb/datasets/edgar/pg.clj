...
(comment
  ;; Load EDGAR into the demo Postgres (edgar/docker-compose.yml — db `edgar`,
  ;; schema owned by init/postgres/01-edgar.sql). Get the dataset first: sync it
  ;; down (scripts/download-dataset.sh --edgar) or produce it locally
  ;; (xtdb.datasets.edgar.mirror/mirror!) — both leave transit under the dir below.
  ;;
  ;; For the full federation demo (node + attach + query), see the comment at the
  ;; bottom of xtdb.datasets.edgar, and modules/datasets/edgar/README.md.
  (def conn
    (jdbc/get-connection {:dbtype "postgresql" :dbname "edgar"
                          :host "localhost" :port 5432
                          :user "postgres" :password "postgres"}))

  ;; per-quarter progress at :info:
  (user/set-log-level! 'xtdb.datasets.edgar.pg :debug)

  (submit-edgar-pg! conn (edgar/dataset "src/test/resources/data/edgar"))

  ;; eyeball the result (psql):
  ;;   SELECT count(*) FROM income_statement;
  ;;   SELECT cik, period_end, net_income_loss, filed FROM income_statement LIMIT 5;
  ;;   SELECT cik, period_end, assets FROM balance_sheet LIMIT 5;
