  ...
  (-> '[:assign [PartSupp [:join [{s_suppkey ps_suppkey}]
                           [:join [{n_nationkey s_nationkey}]
                            [:join [{n_regionkey r_regionkey}]
                             [:scan {:for-valid-time [:at :now] :table public/nation} [n_name n_regionkey n_nationkey]]
                             [:scan {:for-valid-time [:at :now] :table public/region} [r_regionkey {r_name (= r_name ?region)}]]]
                            [:scan {:for-valid-time [:at :now] :table public/supplier} [s_nationkey s_suppkey s_acctbal s_name s_address s_phone s_comment]]]
                           [:scan {:for-valid-time [:at :now] :table public/partsupp} [ps_suppkey ps_partkey ps_supplycost]]]]
        [:top {:limit 100}
           ...
           [:join [{s_nationkey c_nationkey}]
            [:join [{n_nationkey s_nationkey}]
             [:join [{r_regionkey n_regionkey}]
              [:scan {:for-valid-time [:at :now] :table public/region} [{r_name (= r_name "ASIA")} r_regionkey]]
               ...
               [:join [{l_orderkey o_orderkey}]
                [:join [{l_suppkey s_suppkey}]
                 [:join [{p_partkey l_partkey}]
                  [:scan {:for-valid-time [:at :now] :table public/part} [p_partkey {p_type (= p_type ?type)}]]
                  [:scan {:for-valid-time [:at :now] :table public/lineitem} [l_orderkey l_extendedprice l_discount l_suppkey l_partkey]]]
                 [:scan {:for-valid-time [:at :now] :table public/supplier} [s_suppkey s_nationkey]]]
                [:scan {:for-valid-time [:at :now] :table public/orders}
                 [o_orderkey o_custkey
                  {o_orderdate (and (>= o_orderdate ?start_date)
            ...
            [:join [{l_orderkey o_orderkey}]
             [:join [{l_suppkey s_suppkey}]
              [:join [{l_partkey ps_partkey} {l_suppkey ps_suppkey}]
               [:join [{p_partkey l_partkey}]
                [:scan {:for-valid-time [:at :now] :table public/part} [p_partkey {p_name (like p_name "%green%")}]]
                [:scan {:for-valid-time [:at :now] :table public/lineitem}
                 [l_orderkey l_extendedprice l_discount l_suppkey l_partkey l_quantity]]]
               [:scan {:for-valid-time [:at :now] :table public/partsupp} [ps_partkey ps_suppkey ps_supplycost]]]
              [:scan {:for-valid-time [:at :now] :table public/supplier} [s_suppkey s_nationkey]]]
...
(def q11-important-stock-identification
  (-> '[:assign [PartSupp [:project [ps_partkey {value (* ps_supplycost ps_availqty)}]
                           [:join [{s_suppkey ps_suppkey}]
                            [:join [{n_nationkey s_nationkey}]
                             [:scan {:for-valid-time [:at :now] :table public/nation} [n_nationkey {n_name (= n_name ?nation)}]]
                             [:scan {:for-valid-time [:at :now] :table public/supplier} [s_nationkey s_suppkey]]]
                            [:scan {:for-valid-time [:at :now] :table public/partsupp} [ps_partkey ps_suppkey ps_supplycost ps_availqty]]]]]
        ...
        [:group-by [l_shipmode
         ...
         [:project [l_shipmode
                    ...
                    {low_line (if (and (<> o_orderpriority "1-URGENT")
                                ...
                                0)}]
          [:join [{o_orderkey l_orderkey}]
           [:scan {:for-valid-time [:at :now] :table public/orders} [o_orderkey o_orderpriority]]
        ...
        [:order-by [[o_totalprice {:direction :desc}] [o_orderdate {:direction :desc}]]
         [:group-by [c_name c_custkey o_orderkey o_orderdate o_totalprice {sum_qty (sum l_quantity)}]
          [:join [{o_orderkey l_orderkey}]
           [:join [{o_custkey c_custkey}]
            [:semi-join [{o_orderkey l_orderkey}]
             [:scan {:for-valid-time [:at :now] :table public/orders} [o_orderkey o_custkey o_orderdate o_totalprice]]
             [:select (> sum_qty ?qty)
        ...
        [:project [{disc_price (* l_extendedprice (- 1 l_discount))}]
         [:select (or (and (= p_brand ?brand1)
                      ...
                      (and (= p_brand ?brand3)
                           ...
                           (<= p_size 15)))
          [:join [{p_partkey l_partkey}]
           [:scan {:for-valid-time [:at :now] :table public/part} [p_partkey p_brand p_container p_size]]
           [:scan {:for-valid-time [:at :now] :table public/lineitem}
  ...
  (-> '[:order-by [[s_name]]
        [:project [s_name s_address]
         [:semi-join [{s_suppkey ps_suppkey}]
          [:join [{n_nationkey s_nationkey}]
           [:scan {:for-valid-time [:at :now] :table public/nation} [{n_name (= n_name ?nation)} n_nationkey]]
           [:scan {:for-valid-time [:at :now] :table public/supplier} [s_name s_address s_nationkey s_suppkey]]]
          [:join [{ps_partkey l_partkey} {ps_suppkey l_suppkey} (> ps_availqty sum_qty)]
           [:semi-join [{ps_partkey p_partkey}]
            [:scan {:for-valid-time [:at :now] :table public/partsupp} [ps_suppkey ps_partkey ps_availqty]]
            [:scan {:for-valid-time [:at :now] :table public/part} [p_partkey {p_name (like p_name "forest%")}]]]
  ...
  (-> '[:assign [L1 [:join [{l1/l_orderkey l2/l_orderkey} (<> l1/l_suppkey l2/l_suppkey)]
                     [:join [{l1/l_suppkey s_suppkey}]
                      [:join [{l1/l_orderkey o_orderkey}]
                       ...
                       [:scan {:for-valid-time [:at :now] :table public/orders} [o_orderkey {o_orderstatus (= o_orderstatus "F")}]]]
                      [:semi-join [{s_nationkey n_nationkey}]
                       [:scan {:for-valid-time [:at :now] :table public/supplier} [s_nationkey s_suppkey s_name]]
                       [:scan {:for-valid-time [:at :now] :table public/nation} [n_nationkey {n_name (= n_name ?nation)}]]]]
                     [:rename l2
                      [:scan {:for-valid-time [:at :now] :table public/lineitem} [l_orderkey l_suppkey]]]]]
        [:top {:limit 100}
...
(def q22-global-sales-opportunity
  (-> '[:assign [Customer [:semi-join [{cntrycode cntrycode}]
                           [:project [c_custkey {cntrycode (substring c_phone 1 2)} c_acctbal]
                            [:scan {:for-valid-time [:at :now] :table public/customer} [c_custkey c_phone c_acctbal]]]
                           [:table ?cntrycodes]]]
        [:order-by [[cntrycode]]
