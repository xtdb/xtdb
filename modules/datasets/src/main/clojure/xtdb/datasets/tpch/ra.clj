        ...
        [:group-by [l_returnflag l_linestatus
         ...
         [:project [l_returnflag l_linestatus l_shipdate l_quantity l_extendedprice l_discount l_tax
                    ...
                    {charge (* (* l_extendedprice (- 1 l_discount))
                               (+ 1 l_tax))}]
          [:scan {:for-valid-time [:at :now] :table lineitem}
          ...
          [:project [l_orderkey o_orderdate o_shippriority
           ...
           [:join [{o_orderkey l_orderkey}]
            [:join [{c_custkey o_custkey}]
             [:scan {:for-valid-time [:at :now] :table customer} [c_custkey {c_mktsegment (= c_mktsegment ?segment)}]]
             [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey o_custkey o_shippriority
                     {o_orderdate (< o_orderdate ?date)}]]]
            [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_extendedprice l_discount
        ...
        [:group-by [o_orderpriority {order_count (count-star)}]
         [:semi-join [{o_orderkey l_orderkey}]
          ...
          [:select (< l_commitdate l_receiptdate)
           [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_commitdate l_receiptdate]]]]]]
        ...
        [:group-by [n_name {revenue (sum disc_price)}]
         [:project [n_name {disc_price (* l_extendedprice (- 1 l_discount))}]
          [:join [{o_orderkey l_orderkey} {s_suppkey l_suppkey}]
           [:join [{s_nationkey c_nationkey}]
            [:join [{n_nationkey s_nationkey}]
             [:join [{r_regionkey n_regionkey}]
              [:scan {:for-valid-time [:at :now] :table region} [{r_name (= r_name "ASIA")} r_regionkey]]
           ...
           [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_extendedprice l_discount l_suppkey]]]]]]
               ...
               [:join [{s_suppkey l_suppkey}]
                ...
                [:scan {:for-valid-time [:at :now] :table lineitem}
                 [l_orderkey l_extendedprice l_discount l_suppkey
                  {l_shipdate (and (>= l_shipdate ?start-date)
                                   (<= l_shipdate ?end-date))}]]]
               [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey o_custkey]]]
              [:rename n1
               [:scan {:for-valid-time [:at :now] :table nation} [{n_name (or (= n_name ?nation1) (= n_name ?nation2))} n_nationkey]]]]
             [:scan {:for-valid-time [:at :now] :table customer} [c_custkey c_nationkey]]]
            [:rename n2
             [:scan {:for-valid-time [:at :now] :table nation} [{n_name (or (= n_name ?nation1) (= n_name ?nation2))} n_nationkey]]]]]]]]
             ...
             [:join [{c_nationkey n1_n_nationkey}]
               ...
               [:join [{l_orderkey o_orderkey}]
                [:join [{l_suppkey s_suppkey}]
                 [:join [{p_partkey l_partkey}]
                  [:scan {:for-valid-time [:at :now] :table part} [p_partkey {p_type (= p_type ?type)}]]
                  [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_extendedprice l_discount l_suppkey l_partkey]]]
                 [:scan {:for-valid-time [:at :now] :table supplier} [s_suppkey s_nationkey]]]
                [:scan {:for-valid-time [:at :now] :table orders}
                 [o_orderkey o_custkey
                  {o_orderdate (and (>= o_orderdate ?start-date)
              ...
              [:join [{r_regionkey n1_n_regionkey}]
               ...
               [:rename n1
                [:scan {:for-valid-time [:at :now] :table nation} [n_name n_nationkey n_regionkey]]]]]
             [:rename n2
              [:scan {:for-valid-time [:at :now] :table nation} [n_name n_nationkey]]]]]]]]]
            ...
            [:join [{l_orderkey o_orderkey}]
             [:join [{l_suppkey s_suppkey}]
              [:join [{l_partkey ps_partkey} {l_suppkey ps_suppkey}]
               [:join [{p_partkey l_partkey}]
                [:scan {:for-valid-time [:at :now] :table part} [p_partkey {p_name (like p_name "%green%")}]]
                [:scan {:for-valid-time [:at :now] :table lineitem}
                 [l_orderkey l_extendedprice l_discount l_suppkey l_partkey l_quantity]]]
               [:scan {:for-valid-time [:at :now] :table partsupp} [ps_partkey ps_suppkey ps_supplycost]]]
              [:scan {:for-valid-time [:at :now] :table supplier} [s_suppkey s_nationkey]]]
             [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey o_orderdate]]]
            [:scan {:for-valid-time [:at :now] :table nation} [n_name n_nationkey]]]]]]]
      #_
         ...
         [:group-by [c_custkey c_name c_acctbal c_phone n_name c_address c_comment
          ...
          [:project [c_custkey c_name c_acctbal c_phone n_name c_address c_comment
           ...
           [:join [{c_nationkey n_nationkey}]
            [:join [{o_orderkey l_orderkey}]
             [:join [{c_custkey o_custkey}]
              [:scan {:for-valid-time [:at :now] :table customer} [c_custkey c_name c_acctbal c_address c_phone c_comment c_nationkey]]
              [:scan {:for-valid-time [:at :now] :table orders}
               [o_orderkey o_custkey
                {o_orderdate (and (>= o_orderdate ?start-date)
                                  (< o_orderdate ?end-date))}]]]
            ...
            [:scan {:for-valid-time [:at :now] :table nation} [n_nationkey n_name]]]]]]]
...
(def q11-important-stock-identification
  (-> '[:assign [PartSupp [:project [ps_partkey {value (* ps_supplycost ps_availqty)}]
                           [:join [{s_suppkey ps_suppkey}]
                            [:join [{n_nationkey s_nationkey}]
                             [:scan {:for-valid-time [:at :now] :table nation} [n_nationkey {n_name (= n_name ?nation)}]]
                             [:scan {:for-valid-time [:at :now] :table supplier} [s_nationkey s_suppkey]]]
                            [:scan {:for-valid-time [:at :now] :table partsupp} [ps_partkey ps_suppkey ps_supplycost ps_availqty]]]]]
        ...
        [:group-by [l_shipmode
         ...
         [:project [l_shipmode
                    ...
                    {low_line (if (and (<> o_orderpriority "1-URGENT")
                                ...
                                0)}]
          [:join [{o_orderkey l_orderkey}]
           [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey o_orderpriority]]
        ...
        [:group-by [c_count {custdist (count-star)}]
         [:group-by [c_custkey {c_count (count o_comment)}]
          [:left-outer-join [{c_custkey o_custkey}]
           ...
           [:scan {:for-valid-time [:at :now] :table orders} [{o_comment (not (like o_comment "%special%requests%"))} o_custkey]]]]]]
      #_
  ...
  (-> '[:project [{promo_revenue (* 100 (/ promo_revenue revenue))}]
        [:group-by [{promo_revenue (sum promo_disc_price)}
         ...
         [:project [{promo_disc_price (if (like p_type "PROMO%")
                    ...
                    {disc_price (* l_extendedprice (- 1 l_discount))}]
          [:join [{p_partkey l_partkey}]
           [:scan {:for-valid-time [:at :now] :table part} [p_partkey p_type]]
           [:scan {:for-valid-time [:at :now] :table lineitem}
        ...
        [:project [s_suppkey s_name s_address s_phone total_revenue]
         [:join [{total_revenue max_total_revenue}]
          [:join [{supplier_no s_suppkey}]
           Revenue
           [:scan {:for-valid-time [:at :now] :table supplier} [s_suppkey s_name s_address s_phone]]]
          ...
          [:join [{l_partkey l_partkey}]
           [:project [l_partkey {small_avg_qty (* 0.2 avg_qty)}]
            [:group-by [l_partkey {avg_qty (avg l_quantity)}]
             [:scan {:for-valid-time [:at :now] :table lineitem} [l_partkey l_quantity]]]]
           [:scan {:for-valid-time [:at :now] :table lineitem} [l_partkey l_quantity]]]]]]
        ...
        [:order-by [[o_totalprice {:direction :desc}] [o_orderdate {:direction :desc}]]
         [:group-by [c_name c_custkey o_orderkey o_orderdate o_totalprice {sum_qty (sum l_quantity)}]
          [:join [{o_orderkey l_orderkey}]
           [:join [{o_custkey c_custkey}]
            [:semi-join [{o_orderkey l_orderkey}]
             [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey o_custkey o_orderdate o_totalprice]]
             [:select (> sum_qty ?qty)
              [:group-by [l_orderkey {sum_qty (sum l_quantity)}]
               [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_quantity]]]]]
            [:scan {:for-valid-time [:at :now] :table customer} [c_name c_custkey]]]
           [:scan {:for-valid-time [:at :now] :table lineitem} [l_orderkey l_quantity]]]]]]
      (with-params {'?qty 300})))
        ...
        [:project [{disc_price (* l_extendedprice (- 1 l_discount))}]
         [:select (or (and (= p_brand ?brand1)
                      ...
                      (and (= p_brand ?brand3)
                           ...
                           (<= p_size 15)))
          [:join [{p_partkey l_partkey}]
           [:scan {:for-valid-time [:at :now] :table part} [p_partkey p_brand p_container p_size]]
           [:scan {:for-valid-time [:at :now] :table lineitem}
  ...
  (-> '[:order-by [[s_name]]
        [:project [s_name s_address]
         [:semi-join [{s_suppkey ps_suppkey}]
          [:join [{n_nationkey s_nationkey}]
           [:scan {:for-valid-time [:at :now] :table nation} [{n_name (= n_name ?nation)} n_nationkey]]
           [:scan {:for-valid-time [:at :now] :table supplier} [s_name s_address s_nationkey s_suppkey]]]
          [:join [{ps_partkey l_partkey} {ps_suppkey l_suppkey} (> ps_availqty sum_qty)]
           [:semi-join [{ps_partkey p_partkey}]
            [:scan {:for-valid-time [:at :now] :table partsupp} [ps_suppkey ps_partkey ps_availqty]]
            [:scan {:for-valid-time [:at :now] :table part} [p_partkey {p_name (like p_name "forest%")}]]]
  ...
  (-> '[:assign [L1 [:join [{l1_l_orderkey l2_l_orderkey} (<> l1_l_suppkey l2_l_suppkey)]
                     [:join [{l1_l_suppkey s_suppkey}]
                      [:join [{l1_l_orderkey o_orderkey}]
                       ...
                       [:scan {:for-valid-time [:at :now] :table orders} [o_orderkey {o_orderstatus (= o_orderstatus "F")}]]]
                      [:semi-join [{s_nationkey n_nationkey}]
                       [:scan {:for-valid-time [:at :now] :table supplier} [s_nationkey s_suppkey s_name]]
                       [:scan {:for-valid-time [:at :now] :table nation} [n_nationkey {n_name (= n_name ?nation)}]]]]
                     [:rename l2
