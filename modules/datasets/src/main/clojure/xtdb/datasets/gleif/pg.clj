...
(comment
  ;; Load GLEIF into the demo Postgres (xt26-demo/docker-compose.yml — db `gleif`,
  ...
  ;;
  ;; For the full federation demo (node + attach + watch CDC), see the `xt26` dev
  ;; namespace, which drives this loader from a running XTDB node.
  (def conn
  ...
  (submit-gleif-pg! conn (gleif/dataset "src/dev/resources/data/gleif"))
