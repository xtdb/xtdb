 ...
 * ```
 *
 * See modules/datasets/gleif/README.md, and the `(comment ...)` at the bottom of the
 * xtdb.datasets.gleif namespace, for the end-to-end run.
 */
class GleifPgIndexer : PgIndexer {
    ...
    private fun writeOp(openTx: OpenTx, op: RowOp) {
        ...
        when (op) {
            is RowOp.Put -> {
                ...
                // null valid-from → systemTime (the entity family, current state).
            ...
            is RowOp.Delete -> {
                ...
                table.writeDelete(deriveId(op.table, op.row))
            }
        }
    }
    ...
    private fun validFrom(table: String, row: Map<String, Any?>): Instant? = when {
        ...
        else -> row.instantOrNull("effective_date")
    }
    ...
    class Factory : PgIndexer.Factory {
        override fun open(): PgIndexer = GleifPgIndexer()
    ...
    private companion object {
        ...
        fun Map<String, Any?>.instantOrNull(col: String): Instant? =
            when (val v = this[col]) {
                null -> null
                is ZonedDateTime -> v.toInstant()
                is OffsetDateTime -> v.toInstant()
