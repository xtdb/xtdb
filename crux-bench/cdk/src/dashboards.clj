...
(defn ->alarm-widget [m alarm-kw]
  (.. (AlarmWidget$Builder/create)
      (title (String/join "-" [(:bench-type m) (:bench-ns m) (:crux-node-type m)]))
...
(defn update-in-grid [grid k f]
  (w/postwalk #(if (map? %) (assoc % k (f %)) %) grid))

(defn get-grid [grid k]
  (w/postwalk #(if (get % k) (get % k) %) grid))
...
(def env (.. (Environment$Builder.)
             (region "eu-west-2")
             (build)))

(def stack (.. (Stack$Builder/create app "crux-bench-dashboard-stack")
               (env env)
               (build)))

(defn -main []
  (let [ingest-grid (-> ingest-grid
                        (update-in-grid :minute-metric (fn [m] (-> (->metric m) (->math "time/6000" "time-taken(mins)"))))
                        (update-in-grid :rate-metric   (fn [m] (-> (->metric m) (->math "RATE(time)^2" "rate(time-taken(ms))^2"))))
                        (update-in-grid :timing-widget (fn [m] (->graph-widget m [:minute-metric])))
