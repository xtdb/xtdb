...
(defn test-10-most-recent-battery-readings-for-charging-devices [node]
  (try
    (let [start-time (System/currentTimeMillis)
          query '{:find [time device-id battery-temperature]
...
(defn test-busiest-devices-1-min-avg-whose-battery-level-is-below-33-percent-and-is-not-charging [node]
  (try
    (let [start-time (System/currentTimeMillis)
          query '{:find [time device-id cpu-avg-1min battery-level battery-status model]
                  :where [[r :reading/time time]
                          [r :reading/device-id device-id]
