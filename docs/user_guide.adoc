...
As Crux is in development, the best route is to clone the https://github.com/juxt/crux[GitHub repo]. Once in the repo, run

----
lein install
----

For development purposes, we will run Crux entirely in 'embedded'
mode. This means that we will run components such as Kafka and RocksDB
in process.
...
----
<1> You could modify this, if you want to run Kafka outside the local process.
