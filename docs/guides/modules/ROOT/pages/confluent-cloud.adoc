...
Note that the node's indexes will be in-memory by default and therefore will be recomputed each time you start a node. For persisted indexes that resume when you restart a node you probably want to configure xref:{page-component-version}@storage::rocksdb.adoc[RocksDB] as well.
