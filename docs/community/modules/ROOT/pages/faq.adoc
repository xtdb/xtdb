...
(with-open [node (crux/start-node {})]
  (crux/submit-tx
    node
    [[:crux.tx/put      {:xt/id :a, :a "a"}]])
  (crux/submit-tx
    node
    ; This won't match. Either the match should come first or it should match :a "b"
    [[:crux.tx/put      {:xt/id :a, :a "b"}]
     [:crux.tx/match :a {:xt/id :a, :a "a"}]])
  (crux/sync node)
  (crux/entity (crux/db node) :a)) ; {:xt/id :a, :a "a"}
----
...
+
Write consistency across nodes is provided via the `:crux.tx/match` operation.
