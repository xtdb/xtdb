...
The smallest on-disk Crux configuration involves three RocksDB instances, one for each Crux store. The RocksDB Module documentation contains xref:reference::rocksdb.adoc#_using_rocksdb[an example of configuring a store to use RocksDB].
