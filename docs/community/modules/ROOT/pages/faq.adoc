...
Look for https://github.com/xtdb/xtdb/search?p=2&q=logback&unscoped_q=logback[code examples] and https://github.com/xtdb/xtdb/blob/0ac595a6df9efcde57e85356e0edfccf00e85248/crux-bench/logback.xml[config examples] in the Crux repo.
...
The only constraint Crux places on entity ids (`:xt/id`) is that they must be
