...
To start up a node with a connected Crux console and HTTP Server:
...
[#config-start-console]
== Console
Crux offers the Crux console, a web app allowing you to query and submit data to Crux and see data from different perpectives with an easy to use interface.

.Console configuration
[#table-conversion%header,cols="d,d,d"]
|===
|Property|Description|Default Value
|`:crux.console/crux-port` | The port to run the node's HTTP server on | 8080
|`:crux.console/console-port`| The port which the console frontend is started on | 5000
|`:crux.console/routes-prefix` | The console route's prefix | "/console"
|===
[#console-dependency]
*Project Dependency*
...
[source,clj]
----
include::./deps.edn[tags=ConsoleDep]
----
[#console-start]
*Getting started*

The following code creates a default `crux.standalone` running entirely in memory, and starts up a console which can be connected to on `localhost:5000/console`.
[source,clj]
----
include::./src/docs/examples.clj[tags=include-crux-api]

include::./src/docs/examples.clj[tags=start-console-node]
----
...
|`:crux.metrics.dropwizard.csv/report-frequency`|`int`| Interval in seconds between file write
