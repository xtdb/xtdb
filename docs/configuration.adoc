...
To include rocksdb metrics in monitoring `:crux.kv.rocksdb/metrics?` should be
set as `true` and `crux.kv.rocksdb.metrics/metrics-module` needs to be required
as a topology.
...
By default `indexer`, `kv-store`, and `query` metrics are included. It is also
...
|`:crux.metrics/with-kv-metrics?`|`boolean`|`true`| Includes kv-store metrics in the metrics registry
...
(api/start-node {:crux.node/topology ['crux.standalone/topology
                                      'crux.kv.rocksdb.metrics/metrics-modlue
                                      ...]
                 :crux.kv.rocksdb/metrics? true
                 ...})
----
