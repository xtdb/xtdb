...
|`:crux.metrics.dropwizard.prometheus/exclude-metrics`|`string-list`| A list of strings for a gauge black list. This filters out any metrics that are listed.
|`:crux.metrics.dropwizard.prometheus/include-metrics`|`string-list`| A list of strings for a gauge white list. This filters out any metrics that are not listed. If `exclude-metrics` is also include, this overrides that option.
|===
