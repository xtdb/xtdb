...
----
(api/start-node {:crux.node/topology ['crux.standalone/topology
                                      'crux.metrics/with-prometheus-exporter]
                 ...
                 })
----
...
|===
|Field|Property|Description
|`:crux.metrics.dropwizard.prometheus/server-port`|`int`| Desired port number for prometheus client server. Defaults to `8080`
...
|===
|Field|Property|Description
|`:crux.metrics.dropwizard.prometheus/duration`|`string`| Time in ISO-8601 standard between metrics push. Defaults to "PT1S".
