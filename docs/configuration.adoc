...
- https://github.com/juxt/crux/tree/master/crux-docker[Crux HTTP Node]: An image of a Crux HTTP node, useful if you wish to a freestanding Crux node accessible over HTTP, only having to use Docker. Allows you to customize the configuration of the node & logging, and optionally opens an nREPL/pREPL port.
