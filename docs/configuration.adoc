...
|===
|Property|Description|Default value
|`:crux.kafka/bootstrap-servers`|URL for connecting to Kafka i.e. \"kafka-cluster-kafka-brokers.crux.svc.cluster.local:9092\"|*localhost:9092*
...
|`:crux.kafka/group-id`|Kafka client group.id|*(Either environment variable "HOSTNAME"/"COMPUTERNAME", or a random UUID)*
