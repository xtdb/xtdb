...
In the `mvn-uberjar` directory, add dependencies to the `pom.xml` file, update the `xtdb.edn` file as before, and then run `build-uberjar.sh` to create the uberjar. To run the maven generated uberjar, use the following command: `java -jar crux.jar`
