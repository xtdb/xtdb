...
Lucene, like Crux, indexes transactions asynchronously.
Once you've indexed transactions into Crux in the usual way, you'll need to call `(crux.lucene/await-tx node tx)` (in addition to `crux.api/await-tx`) in order to read your own writes - this ensures Lucene has indexed the given transaction.
