...
{
  "xt/document-store": {
    "xt/module": "xtdb.google.cloud-storage/->document-store",
...
[source,clojure]
----
{:xt/document-store {:xt/module 'xtdb.google.cloud-storage/->document-store
                       :root-path "gs://bucket/prefix"}}
----
...
{:checkpointer {...
                :store {:xt/module 'xtdb.google.cloud-storage/->checkpoint-store
