...
{
  "xt/document-store": {
    "xt/module": "xtdb.s3/->document-store",
    "bucket": "your-bucket",
    ...
  },
}
...
[source,clojure]
----
{:xt/document-store {:xt/module 'xtdb.s3/->document-store
                     :bucket "your-bucket"
                     ...}}
----
...
{:checkpointer {...
                :store {:xt/module 'xtdb.s3.checkpoint-store/->checkpoint-store
                        :configurator ...
                        :bucket "..."
                        :prefix "..."}}
----
...
We expose these via the https://github.com/juxt/crux/blob/master/modules/s3/src/xtdb/s3/S3Configurator.java[`xtdb.s3.S3Configurator`] interface - you can supply an instance using the following in your node configuration.
