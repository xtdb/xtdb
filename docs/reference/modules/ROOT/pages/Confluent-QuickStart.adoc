...
Use `confluent-hub install juxt/kafka-connect-crux:{crux_version}` to download and install the connector from Confluent hub. The downloaded connector is then placed within your confluent install's _'share/confluent-hub-components'_ folder. +
...
[source,clj]
----
(crux/submit-tx node [[:xt/put {:xt/id #uuid "415c45c9-7cbe-4660-801b-dab9edc60c82", :value "baz-source"}]])
----
