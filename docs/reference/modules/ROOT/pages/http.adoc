...
Crux offers a REST API layer in the `xtdb-http-server` module that allows you to send transactions and run queries over HTTP.
...
The REST API also provides an experimental endpoint for SPARQL 1.1 Protocol queries under `/_xtdb/sparql/`, rewriting the query into the Crux Datalog dialect.
...
----
com.xtdb/xtdb-http-server {:mvn/version "{crux_version}"}
----
...
[source,clojure]
----
(xtdb.http-server/->crux-handler crux-node {...})
----
...
[source,clojure, subs=attributes+]
----
com.xtdb/xtdb-http-client {:mvn/version "{crux_version}"}
----
...
{:crux.version/version "{crux_version}",
 ...
 :crux.kv/kv-store "xtdb.rocksdb.RocksKv",
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/entity?eid=tommy
----
...
curl -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/entity?eid-edn=:tommy
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/entity?eid=tommy&history=true&sortOrder=asc
----
...
curl -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/entity?eid-edn=:tommy&history=true&sort-order=asc
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/entity-tx?eid=tommy
----
...
curl -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/entity-tx?eid-edn=:tommy
----
====
...
curl -g \
     -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/query?queryEdn={%3Afind+[e]+%3Awhere+[[e+%3Axt/id+_]]}
----
...
curl -g \
     -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/query?query-edn={%3Afind+[e]+%3Awhere+[[e+%3Axt/id+_]]}
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/attribute-stats
----
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/sync?timeout=500
----
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/await-tx?txId=1
----
...
curl -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/await-tx?tx-id=1
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/await-tx-time?tx-time=2020-10-16T14:29:35Z
----
...
({:xt/tx-id 0,
  ...
  :crux.tx.event/tx-events [[:xt/put
...
curl -X POST \
     -H "Content-Type: application/edn" \
     -H "Accept: application/edn" \
     -d '{:tx-ops [[:xt/put {:xt/id :ivan, :name "Ivan" :last-name "Petrov"}],
          	       [:xt/put {:xt/id :boris, :name "Boris" :last-name "Petrov"}],
          	       [:xt/delete :maria #inst "2012-05-07T14:57:08.462-00:00"]]}' \
     $CRUX_URL/_xtdb/submit-tx
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/tx-committed?txId=1
----
...
curl -X GET \
     -H "Accept: application/edn" \
     $CRUX_URL/_xtdb/tx-committed?tx-id=1
----
====
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/latest-completed-tx
----
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/latest-submitted-tx
----
...
curl -X GET \
     -H "Accept: application/json" \
     $CRUX_URL/_xtdb/slowest-queries
----
