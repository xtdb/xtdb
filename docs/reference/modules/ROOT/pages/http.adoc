...
* `start-valid-time`, `start-transaction-time`, `start-transaction-id` (inclusive, default unbounded): bitemporal co-ordinates to start at
* `end-valid-time`, `end-transaction-time`, `end-transaction-id` (exclusive, default unbounded): bitemporal co-ordinates to stop at
...
.*Optional Parameters*
* `valid-time` (date, defaulting to now)
* `transact-time` (date, defaulting to latest transaction time)
* `tx-id` (date, defaulting to latest transaction id)
