...
The `:where` section of a query constrains the result set to match only entities (or subgraphs of interconnected entities) which satisfy all clauses.
Clauses are edn vectors within the `:where` vector.

[#where-existence]
=== Existence

A clause which consists of a logic variable and a keyword is satisfied if that keyword exists.
...
This matches all entities, `p`, which have a `:name`

[#where-tuple]
=== Tuple

A clause which consists of a logic variable, a keyword and a third value is satisfied if that tuple exists.
...
This matches all entities, `p`, which have a `:name` of `"Smith"`.

The third value can be a logic variable which is constrained by other clauses.
...
This matches all entities, `p`, which have a `:father` of `f`.

`f` is restricted to only entities which have a `:name` of `"Smith"`.

So, overall, this returns all people who have a father named Smith.

[#where-predicates]
=== Predicates

A fully qualified Clojure function that returns a boolean can be used as a predicate clause
...
This matches all entities, `p` which have an `:age` of at least 18.
