...
Crux queries can take a set of additional arguments, binding them to variables under the `:in` key within the query. Below we have some examples of queries with the various kinds of bindings that `:in` supports (ran on the data as entered into the database above).
...
In this case, we have a list of ages as our input, and only `22` satisfies the predicate, `(> age 21)`.
...
Any fully qualified Clojure function that returns a boolean can be used in place of `>`, for example:
...
Within Crux queries, you can nest a subquery - and bind the results for further use in the query.
...
[source,clj]
----
include::example$src/docs/examples.clj[tags=rules]
----
