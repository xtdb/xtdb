...
//See the https://github.com/xtdb/xtdb/blob/master/test/test/crux/query_test.clj[`Crux query test file`] for the full suite of query tests and how each test listed below runs in the wider context.
...
Extensible Data Notation (edn) is a simple data format that is used to describe Crux Datalog queries.
...
Many examples of advanced queries are showcased across the https://github.com/xtdb/xtdb/blob/master/test/test/crux/query_test.clj[query tests] and the suite of benchmarks maintained in the `crux-bench` sub-project.
