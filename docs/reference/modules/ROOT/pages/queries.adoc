...
//This page walks through many of the more interesting queries that run as part of XTDB's default test suite.
//See the https://github.com/xtdb/xtdb/blob/master/test/test/xtdb/query_test.clj[`XTDB query test file`] for the full suite of query tests and how each test listed below runs in the wider context.
...
- `get-else` (see https://github.com/xtdb/xtdb/blob/7f7b50fcc7c766c8e5913ea7195138c9d1e5314a/crux-test/test/crux/query_test.clj#L1056[`get-attr`] which returns a relation instead)
- `get-some`
...
Many examples of advanced queries are showcased across the https://github.com/xtdb/xtdb/blob/master/test/test/crux/query_test.clj[query tests] and the suite of benchmarks maintained in the `bench` sub-project.
