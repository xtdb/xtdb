...
When calling `submitTx`, the return value is a `TransactionInstant`.

This `TransactionInstant` can be used for checking whether a given Transaction has been indexed yet, or for waiting until it has been indexed.

It contains the `transactionId` and `transactionTime` of the submitted Transaction.
