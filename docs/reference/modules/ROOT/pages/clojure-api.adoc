...
----
  (db
    ...
    [node ^Date valid-time ^Map transaction]
    ^:deprecated [node ^Date valid-time ^Date transaction-time]
    "When a valid time is specified then returned db value contains only those
     ...
     When both valid and transaction time/transaction are specified returns a db value as of
     the valid time and the latest transaction indexed at or before the
...
----
  (open-db
    ...
    "When a valid time is specified then returned db value contains only those
     ...
     When both valid time and transaction are specified returns a db value as of
     the valid time and the latest transaction indexed at or before the
     specified transaction.
