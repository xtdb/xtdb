...
+
--
A transaction is performed by calling `crux.api/submit-tx` on a node with a list of transaction operations.
...
Persistence of Clojure metadata is not supported (although is not currently rejected either) - we strongly recommended that all metadata should be removed from each document prior to submission to Crux, to avoid potential equality checking issues (see https://github.com/juxt/crux/issues/1510[here] for more details).
--
====
...
|URI|`#xt/id "mailto:crux@juxt.pro"`
|URL|`#xt/id "https://github.com/juxt/crux"`
|Maps|`{:this :id-field}`
|===

The `#xt/id` reader literal will take URI/URL strings and attempt to coerce them into valid IDs.
