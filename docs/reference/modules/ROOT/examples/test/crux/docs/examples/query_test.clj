...
(t/deftest test-rules
  (let [node *api*]
       (put-and-wait node [{:crux.db/id :ivan :follow :petr :age 19}
...
(t/deftest test-blanks
  (let [node *api*]
       (put-and-wait node [{:crux.db/id :ivan :name "Ivan"}
                           {:crux.db/id :petr :name "Petr"}
                           {:crux.db/id :sergei :name "Sergei"}])
...
(t/deftest test-generic
  (let [node *api*]
       (put-and-wait node [{:crux.db/id :ivan :name "Ivan" :last-name "Ivan"}])
