...
local node system when horizontal scalability is required or when it
is necessary to achieve the guarantees that Kafka is capable of
providing in terms of resiliency, availabilty, and retention of
data.

Multiple instances of a local node systems may participate in a
...
|Local Node|Uses Kakfa|`Crux.startClusterNode options;`
...
|Standalone|tx-log|`bootstrap-servers`|Kakfa Servers to connect to e.g. `"localhost:9092"`
...
Set the `server-port` configuration property on a Crux local node
system to expose a HTTP port that will accept REST requests.
