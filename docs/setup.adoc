...
To use Crux you need to build a Crux system. Systems implement the
`ICruxSystem` interface and are the starting point for making use of
Crux. Systems also implement `java.io.Closeable` and can therefore be
lifecycle managed.

Systems are started using the
...
local node system if you want Crux system instances to participate in
