...
NOTE: TODO, drop the approach of doing a `lein install` to play with
Crux. Walk users through the minimal dependency setup to get a
kafka-less instance running so that they can play with it.
...
start a standalone Crux instance as described below, you can then
...
An easy way to fire up a local Kafka and Zookeeper is via
Kafka. Follow
https://docs.confluent.io/current/installation/docker/docs/installation/single-node-client.html[this
guide] on the confluent docs for how to do this.

NOTE: TODO complete this

=== Embedded Kafka

NOTE: TODO, write up the simple case of connecting to a running Kakfa
- go through the steps outlined in
