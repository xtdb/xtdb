...
The `table` operator yields each element as a row, with the values in the map link:#_binding_specs[bound/constrained] as required.
...
* In link:#_table[`table`,role=no-underline] and link:#_from[`from`,role=no-underline] clauses any logic variables specified in its binding specs are unified.
