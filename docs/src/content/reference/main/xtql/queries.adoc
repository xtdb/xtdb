...
+
If the requested transaction hasn't been indexed, the XTDB client will wait (see `:basis-timeout`) before evaluating the query.
+
** `:tx` (overrides `:after-tx`): specifies the _exact_ latest transaction that'll be visible to the query.
*** If not provided, this will default to the latest available transaction on the node.
** `:after-tx`: requires that the node has indexed _at least_ the specified transaction.
...
*** If the requested transaction hasn't been indexed, the XTDB client will wait (see `:basis-timeout`) before evaluating the query.
** `:current-time` (defaults to the current wall-clock time of the executing node): overrides the wall-clock time used in any link:../stdlib/temporal#_current_time[functions] that require it.
+
...
* `:default-tz`: the default timezone to use in link:../stdlib/temporal[functions] that require it - defaults to the JVM timezone of the server.
* `:explain?` (default `false`): rather than returning results, setting this flag to `true` returns the query plan for the query.
