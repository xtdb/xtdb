...
In common SQL databases, the original price '100' is lost forever, and no information about when it was last changed is recorded. Perhaps if you're sufficiently desperate and very lucky there _might_ be an old backup of the data available somewhere with a previous value.
...
Applications can set the `valid_to` column explicitly to correspond with data lifecycle archival events like 'settled_at'. Alternatively, they can simply `DELETE` the relevant records (setting `valid_to` to the implicit `CURRENT_TIMESTAMP` at the time of the transaction):
