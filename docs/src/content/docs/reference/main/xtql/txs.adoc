...
** To specify a valid-time range, the query may return `xt$validFrom` and/or `xt$validTo` columns.
...
* `bind` (link:./queries#_binding_specs[Binding Specs], optional): select the documents to update using binding constraints.
...
* `set` (array of string -> link:./queries#_expressions[Expressions], optional): columns to update.
...
* `argRows` (link:./queries#_argument_specs[Argument Specs], optional): arguments to be used in the `bind`,`set` or `unify` contexts. The update operation will be run for every argument spec input.
====
...
* `bind` (link:./queries#_binding_specs[Binding Specs], optional): select the documents to delete using binding constraints.
...
* `unify` (link:./queries#_unify_clauses[Unify clauses], optional): extra clauses to unify with (e.g. to join against another table).
   See link:./queries#_unify[unify] for more details.
* `argRows` (link:./queries#_argument_specs[Argument Specs], optional): arguments to be used in the `bind`,`set` or `unify` contexts. The delete operation will be run for every argument spec input.
====
...
* `bind` (link:./queries#_binding_specs[Binding Specs], optional): select the documents to erase using binding constraints.
** If not provided, this will delete every document in the table.
** May also bind extra logic variables for use in the `unify-clauses`.
* `unify` (link:./queries#_unify_clauses[Unify clauses], optional): extra clauses to unify with (e.g. to join against another table).
   See link:./queries#_unify[unify] for more details.
* `argRows` (link:./queries#_argument_specs[Argument Specs], optional): arguments to be used in the `bind` or `unify` contexts. The erase operation will be run for every argument spec input.
