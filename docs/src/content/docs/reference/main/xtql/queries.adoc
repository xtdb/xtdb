...
| link:#_order_by[`order-by`,role=no-underline] | Orders the relation by the given columns
...
Additionally `from` supports a special column reference `*`, like in SQL this can be used to specify that all columns of a given table are to be projected out.
...
Without an explicit preceding link:#_order_by[`order-by`,role=no-underline], the rows selected for return are undefined.
