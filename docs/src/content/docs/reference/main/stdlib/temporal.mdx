...
`RANGE(start, end [, stride])` (2.2+)

: Alias for `GENERATE_SERIES` — generates a series of values from the given start (inclusive) to the given end (exclusive), with an optional stride.
