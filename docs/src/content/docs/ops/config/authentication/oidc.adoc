...
XTDB integrates with OpenID Connect identity providers (Keycloak, Auth0, Azure AD) for enterprise authentication. OIDC authentication supports multiple OAuth 2.0 flows for different client types.
...
NOTE: Not all OIDC providers support all OAuth 2.0 flows. PASSWORD and DEVICE_AUTH flows require specific provider support and configuration. Check your provider's documentation for supported grant types.
...
Base64-encoded `client-id:client-secret` used as password.

**Client Usage**::
[source,bash]
----
# Encode credentials
ENCODED=$(echo -n "client-id:client-secret" | base64)
PGPASSWORD="$ENCODED" psql -h localhost -p 5432 -U oidc-client -d xtdb
----
...
3. Client polls XTDB until user completes authentication
...
**Invalid Credentials (Client Credentials)**::
Verify base64 encoding: `echo -n "client-id:client-secret" | base64`
