...
XTDB provides tools and templates to facilitate the monitoring and
observability of XTDB nodes. Metrics are exposed in the **Prometheus**
format, which can be scraped by **Prometheus** and visualized in
**Grafana** using XTDB's pre-built dashboards.

:::note
The XTDB cloud images come pre-configured with Prometheus metrics
exposed - see the ["Monitoring docs"](../config/monitoring) for more
information.
:::
...
[here](https://github.com/xtdb/xtdb/blob/main/monitoring/public-dashboards/xtdb-monitoring.json).
...
Download the JSON template:
[here](https://github.com/xtdb/xtdb/blob/main/monitoring/public-dashboards/xtdb-node-debugging.json).
