...
NOTE: As it will take some time for the XTDB nodes to be marked as ready (as they need to pass their initial startup checks), and AWS will need to provision a loadbalancer for the service, it may take a few minutes for the XTDB cluster to be accessible.
