...
Whilst XTDB offers some level of compatibility with Postgres' actual SQL dialect, there are many syntactic and functional capabilities which Postgres offers that are not supported in XTDB, and users should generally always refer to XTDB's [SQL documentation](/reference/main/sql/queries).
...
const sql = postgres({
  host: "localhost",
  port: 5432,
  fetch_types: false, // currently required to be 'false'
  // debug: console.log
});

// Mapping of domain JavaScript types to Postgres OIDs
const typeToOid = {
  number: 23,  // Postgres OID for 'int4' (note that 20/'int8' will coerce to string by default due JS 53-bit numeric precision)
...
async def insert_trades(conn, trades):
    ...
    trade_values = [(trade["_id"], trade["name"], trade["quantity"]) for trade in trades]

    async with conn.cursor() as cur:
        await cur.executemany(query, trade_values)
...
async def main():
    ...
    try:
        async with await pg.AsyncConnection.connect(**DB_PARAMS,
