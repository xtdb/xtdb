...
Note that you are able to add `:full-results? true` to the query map to easily retrieve the source documents relating to the entites in the result set. For instance to retrieve _all_ documents in a single query:
