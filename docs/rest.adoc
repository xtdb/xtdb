...
Takes a Datalog query and returns its results. Note that you are able to add `:full-results true` to the query map to easily retrieve the source documents relating to the entites in the result set.
