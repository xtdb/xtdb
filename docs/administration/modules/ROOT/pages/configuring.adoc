...
| XTDB_ENABLE_BYTEUTILS_SHA1 | xtdb.enable-byteutils-sha1 | Set to use XT's Java SHA1 implementation rather than system libraries.
| XTDB_DISABLE_LIBCRYPTO | xtdb.disable-libcrypto | Set to disable use of libcrypto (OpenSSL) for SHA1 hashing
| XTDB_DISABLE_LIBGCRYPT | xtdb.disable-libgcrypt | Set to disable use of libgcrypt for SHA1 hashing.
