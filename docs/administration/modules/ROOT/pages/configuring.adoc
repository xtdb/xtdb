...
* xref:_persistence_stores[Persistence Stores]
...
Once you have an xref:installation.adoc[in-memory XTDB node] set up, you can then start to configure the various modules - either through a JSON config file, EDN config file, or programmatically:
...
| xref:persistence-stores::aws-s3.adoc[AWS S3] | | ✓ |
| xref:persistence-stores::azure-blobs.adoc[Azure Blobs] | | ✓ |
| xref:persistence-stores::corda.adoc[DLT - Corda] footnote:[configured via its own entry point - see the module docs for more information] | ✓ | ✓ |
| xref:persistence-stores::google-cloud-storage.adoc[Google Cloud Storage] | | ✓ |
| xref:persistence-stores::kafka.adoc[Kafka] | ✓ | ✓ |
| xref:persistence-stores::jdbc.adoc[JDBC] | ✓ | ✓ |
| In-memory KV | ✓ | ✓ | ✓
| xref:persistence-stores::lmdb.adoc[LMDB] (KV) | ✓ | ✓ | ✓
| xref:persistence-stores::rocksdb.adoc[RocksDB] (KV) | ✓ | ✓ | ✓
| xref:persistence-stores::xodus.adoc[Xodus] (KV) footnote:[via third-party `crux-xodus` module] | ✓ | ✓ | ✓
|===
