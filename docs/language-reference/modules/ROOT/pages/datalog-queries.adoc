= Datalog Queries
:page-aliases: 1.23.3@reference::queries.adoc
...
* Function calls are guarded by xref:{page-component-version}@administration::configuring.adoc#fn-allowlist[`:fn-allow-list`], if it was specified in the node configuration.
...
NOTE: For maximum clarity and performance it is often best to use explicit rules instead of `or`/`or-join` (which are essentially like 'anonymous' rules).
Explicit rules benefit from an ability to specify which vars must be bound before the rule is executed and can therefore be configured to avoid performing excessive work.
...
They can be used as a shorthand for when you would otherwise be repeating the same clauses in your `:where`.
...
Rules and `or-join` both execute internally as non-lazy subqueries whose results are unified (and potentially cross-joined) with the tuples in the outer query.
...
If specified, it overrides the `:query-timeout` xref:{page-component-version}@administration::configuring.adoc#_query_engine[Query Engine configuration] option, which defaults to `30000` (30 seconds).
...
All coordinates can be null. Start coordinates are inclusive and end coordinates are exclusive, however the specified sort-order direction will be relative to the actual order of the start/end times provided.

The returned order is always sorted by `valid-time` first, and then `tx-time` second as the tie-break (e.g. when `with-corrections? true` is specified). `tx-id` is the tie-break when `tx-time` is equal.
