...
|`(max ?lvar)`
|`(count ?lvar)`| Return a single count of all values including any duplicates (note: this always performs a scan - no statistics or other stateful materializations are used)
