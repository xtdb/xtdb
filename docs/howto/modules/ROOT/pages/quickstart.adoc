...
have a single Crux node that persists records to disk using an in-process RocksDB instance.
...
If you are trying Crux for the first time, the details of each store are not important right now. For more advanced configuration, Crux allows you to choose the underlying data storage technology (for each of the three components) from a number of different modules. The Reference Docs contains a
xref:reference::configuration.adoc#_modules[list of available modules], each with instructions on how to configure them.
+++ </div></details> +++
