...
Refer to the https://www.opencrux.com/reference/20.09-1.12.1/clojure-api.html[documentation] if you want to understand the Crux APIs.

You can launch a `clj` REPL and provide the latest `crux-kafka` on Clojars using:
[source,sh]
----
clj -Sdeps '{:deps {juxt/crux-core {:mvn/version "RELEASE"} juxt/crux-kafka {:mvn/version "RELEASE"}}}'
----
