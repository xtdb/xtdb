...
|`crux.tx.EventTxLog`| Uses the KvStore for the transaction log in standalone mode, backed by `moberg`. This is useful when Kafka isn't desired, and the topology of having distributed Crux nodes feeding of a centralised transaction log isn't required.
|===
