...
*A:* No. We have a simple ingestion mechanism for RDF data in `crux.rdf`
...
*Q:* Does Crux support the full Datomic/Datascript dialect of Datalog?

*A:* No. The `:where` part is similar, but only the map form of queries
...
*A:* The goal is to support different languages, and decouple the query
...
*A:* The business time can be set manually per transaction operation,
...
*A:* Not directly. One idea is that aggregates can be implemented on top
