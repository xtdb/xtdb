...
`UnaryJoinVirtualIndex` comprises of multiple child indices. Calling
`seek-values` on it will advance all the child indices internally
until they all contain the same key. This involves calling
`seek-values` on each child index until the indices match at the same
level. Calling `next-values` would move all the indices along until
the next common key that all the indices share.
