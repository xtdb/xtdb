  ...
  (with-open [xt-db-conn (pgw-test/jdbc-conn)]
    ...
    (with-open [new-db-conn (pgw-test/jdbc-conn {:dbname "new_db"})]
      ...
      (t/is (anomalous? [:incorrect ::pgw/invalid-db
                         "CREATE DATABASE is only allowed from the primary database"]
                        (err/wrap-anomaly {}
...
(t/deftest test-drop-database
  (with-open [xt-db-conn (pgw-test/jdbc-conn)]
    ...
    (with-open [new-db-conn (pgw-test/jdbc-conn {:dbname "new_db"})]
      ...
      (jdbc/execute! xt-db-conn ["DROP DATABASE new_db"])

      (t/is (anomalous? [:incorrect :xtdb.database/not-found
                         "Database 'new_db' does not exist"]
                        (err/wrap-anomaly {}
                          (jdbc/execute-one! new-db-conn ["SELECT * FROM foo"])))))))
