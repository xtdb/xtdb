...
(deftest earilier-recency-buckets-can-effect-splitting-in-later-buckets-4097
  (let [[system-from1 system-from2] (mapv inst->micros [#inst "2020-01-01" #inst "2020-01-03"])
        [valid-from1 valid-from2] (mapv inst->micros [#inst "2020-01-01" #inst "2020-01-03"])
        [valid-to1 valid-to2] (mapv inst->micros [#inst "2021-01-05" #inst "2020-01-04"])]
...
(deftest test-to-merge-task
  (let [insts (tu/->instants :day)
        [sf1 sf2 sf3] (->> insts (take 3) (map inst->micros))
        [vf1 vf2 vf3] (->> insts (take 3) (map inst->micros))
        [vt1 vt2 vt3] (->> insts (drop 1) (take 3) (map inst->micros))
        ->pages #(into #{} (map :page) %)]

    ;; First page comes before in system time
    ;; Third page doesn't overlap in valid time
    (t/is (= #{1} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 vt1 sf1 Long/MAX_VALUE sf1)
                                           (->mock-merge-plan-page 1 true vf2 vt2 sf2 Long/MAX_VALUE sf2)
                                           (->mock-merge-plan-page 2 false vf3 vt3 sf3 Long/MAX_VALUE sf3)]
                                          (tu/->min-max-page-bounds vf2 Long/MAX_VALUE sf2 Long/MAX_VALUE))
                       ->pages))
          "No need to take extra pages!")

    (t/is (= #{1 2} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 (inc vt1) sf1 Long/MAX_VALUE sf1)
                                             (->mock-merge-plan-page 1 true vf2 (inc vt2) sf2 Long/MAX_VALUE sf2)
                                             (->mock-merge-plan-page 2 false vf3 (inc vt3) sf3 Long/MAX_VALUE sf3)]
                                            (tu/->min-max-page-bounds vf2 Long/MAX_VALUE sf2 Long/MAX_VALUE))
                         ->pages))
          "We make the valid time windows slightly larger so it overlaps. Third page needs to get taken.")

    (t/is (= #{1 2} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 (inc vt1) sf1 Long/MAX_VALUE sf1)
                                             (->mock-merge-plan-page 1 true vf2 (inc vt2) sf2 Long/MAX_VALUE sf2)
                                             (->mock-merge-plan-page 2 false vf3 (inc vt3) sf3 Long/MAX_VALUE sf3)]
                                            (tu/->min-max-page-bounds vf2 vt2 sf2 Long/MAX_VALUE))
                         ->pages))
          "If a row from page 2 can be bound by page 3, page 3 needs to be taken.")

    (t/is (= #{1} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 (inc vt1) sf1 Long/MAX_VALUE sf1)
                                           (->mock-merge-plan-page 1 true vf2 (inc vt2) sf2 Long/MAX_VALUE sf2)
                                           (->mock-merge-plan-page 2 false vf3 (inc vt3) sf3 Long/MAX_VALUE sf3)]
                                          (tu/->min-max-page-bounds vf2 vt2 sf2 sf3))
                       ->pages))
          "The same as the preivous test but system-time excludes page 3.")

    (t/is (= #{0 1} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 (inc vt1) sf2 Long/MAX_VALUE sf2)
                                             (->mock-merge-plan-page 1 true vf2 vt2 sf2 Long/MAX_VALUE sf2)
                                             (->mock-merge-plan-page 2 false vf3 vt3 sf3 Long/MAX_VALUE sf3)]
                                            (tu/->min-max-page-bounds vf2 Long/MAX_VALUE sf2 Long/MAX_VALUE) )
                         ->pages))
          "Page 0 might contain events after page 1 and overlaps in valid time.")

    (t/is (= #{1} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 (inc vt1) sf2 Long/MAX_VALUE (dec sf2))
                                           (->mock-merge-plan-page 1 true vf2 vt2 sf2 Long/MAX_VALUE sf2)
                                           (->mock-merge-plan-page 2 false vf3 vt3 sf3 Long/MAX_VALUE sf3)]
                                          (tu/->min-max-page-bounds vf2 Long/MAX_VALUE sf2 Long/MAX_VALUE) )
                       ->pages))
          "The same as the previous test but not event can come after page 1.")

    (t/is (= #{1} (->> (trie/->merge-task [(->mock-merge-plan-page 0 false vf1 vt1 sf2 Long/MAX_VALUE sf2)
                                           (->mock-merge-plan-page 1 true vf2 vt2 sf2 Long/MAX_VALUE sf2)
                                           (->mock-merge-plan-page 2 false vf3 vt3 sf3 Long/MAX_VALUE sf3)]
                                          (tu/->min-max-page-bounds vf2 Long/MAX_VALUE sf2 Long/MAX_VALUE) )
                       ->pages))
          "The same as two tests above test but valid-time doesn't overlap with page 1")

    (t/is (= #{0 1 2} (->> (trie/->merge-task [(->mock-merge-plan-page 0 true vf1 Long/MAX_VALUE sf1 Long/MAX_VALUE sf1)
                                               (->mock-merge-plan-page 1 false vf2 vt2 sf2 Long/MAX_VALUE sf2)
                                               (->mock-merge-plan-page 2 false vf3 vt3 sf3 Long/MAX_VALUE sf3)])
                           ->pages))
