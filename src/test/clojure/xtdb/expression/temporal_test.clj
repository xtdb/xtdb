    ...
    (letfn [(test-cast
      ...
      (t/testing "string ->"
        ...
        (t/testing "ts"
          ...
          (t/is (= #time/date-time "2022-08-01T05:34:56.789"
...
(t/deftest test-temporal-arithmetic
  (letfn [(test-arithmetic
    ...
    (t/testing "(+ datetime duration)"
      ...
      (t/is (nil? (test-arithmetic '+ nil #time/duration "PT1H15M43.342S"))))
    ...
    (t/testing "(- datetime datetime)" 
      ...
      (t/is (nil? (test-arithmetic '- nil  #time/date-time "2022-08-01T02:31:26.684")))
      (t/is (nil? (test-arithmetic '- #time/zoned-date-time "2022-08-01T01:15:43.342+01:00[Europe/London]" nil)))
...
(deftest test-period-constructor
  (letfn [(f [from to]
            (et/project1 '(period x y)
                         {:x from, :y to}))]
    (let [from #time/zoned-date-time "2020-01-01T00:00Z"
          to #time/zoned-date-time "2022-01-01T00:00Z"]
      (t/is (= (tu/->tstz-range from to)
               (f from to))))

    (let [from #time/zoned-date-time "2030-01-01T00:00Z"
          to #time/zoned-date-time "2020-01-01T00:00Z"]
      (t/is
       (thrown-with-msg?
        RuntimeException
    ...
    (t/testing "other date-time types"
      (t/is (= #xt/tstz-range [#time/zoned-date-time "2020-01-01T00:00Z"
                               #time/zoned-date-time "2020-01-02T00:00Z"]
               (f #time/date "2020-01-01" #time/date "2020-01-02"))
            "date/date")

      (t/is (= #xt/tstz-range [#time/zoned-date-time "2020-01-01T00:00Z" nil]
               (f #time/date "2020-01-01" nil))
            "date/nil")

      (t/is (thrown-with-msg? IllegalArgumentException #"period not applicable to types null and date"
                              (f nil #time/date "2020-01-01"))
            "nil/date")

      (t/is (= #xt/tstz-range [#time/zoned-date-time "2020-01-01T00:00Z"
                               #time/zoned-date-time "2020-01-02T01:23:45Z"]
               (f #time/date "2020-01-01" #time/date-time "2020-01-02T01:23:45"))
            "date/ts")

      (t/is (= #xt/tstz-range [#time/zoned-date-time "2020-01-01T01:23:45Z"
                               #time/zoned-date-time "2020-01-02T00:00Z"]
               (f #time/date-time "2020-01-01T01:23:45" #time/date "2020-01-02"))
            "ts/date")

      (t/is (= #xt/tstz-range [#time/zoned-date-time "2020-01-01T11:07:08Z"
                               #time/zoned-date-time "2020-01-02T01:23:45Z"]
               (f #time/zoned-date-time "2020-01-01T06:07:08-05:00[America/New_York]",
                  #time/date-time "2020-01-02T01:23:45"))
            "tstz/ts"))))
...
(t/deftest test-period-intersection-3493
  (t/is (= (tu/->tstz-range #time/zoned-date-time "2021-01-01T00:00Z"
  ...
  (t/is (= (tu/->tstz-range #time/zoned-date-time "2020-01-01T00:00Z"
                            #time/zoned-date-time "2022-01-01T00:00Z")
           (et/project1 '(* p1 p2)
                        {:p1 (tu/->tstz-range #inst "2020-01-01", #inst "2022-01-01")
