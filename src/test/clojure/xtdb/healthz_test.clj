    ...
    (let [port (tu/free-port)]
      (with-open [node (tu/->local-node {:node-dir local-path
        ...
        (let [^LiveIndex live-index (tu/component node :xtdb.indexer/live-index)]
          ...
          (let [last-tx-id (:tx-id (xt/execute-tx node [[:put-docs :bar {:xt/id "bar1"}]
            ...
            (t/is (= last-tx-id (:tx-id (.getLatestCompletedBlockTx live-index)))))

          (let [last-tx-id (:tx-id (xt/execute-tx node [[:put-docs :bar {:xt/id "bar7"}]
