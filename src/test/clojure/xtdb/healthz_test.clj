...
(t/deftest test-finish-block-endpoint
  (util/with-tmp-dirs #{local-path}
    (let [port (tu/free-port)]
      (with-open [node (tu/->local-node {:node-dir local-path
                                         :healthz-port port})]
        (let [log (tu/component node :xtdb.api/log)
              ^LiveIndex live-index (tu/component node :xtdb.indexer/live-index)]

          (t/is (= 0 (:tx-id (xt/execute-tx node [[:put-docs :bar {:xt/id "bar1"}]
                                                  [:put-docs :bar {:xt/id "bar2"}]
                                                  [:put-docs :bar {:xt/id "bar3"}]]))))

          (t/is (= 2097 (:tx-id (xt/execute-tx node [[:put-docs :bar {:xt/id "bar4"}]
                                                     [:put-docs :bar {:xt/id "bar5"}]
                                                     [:put-docs :bar {:xt/id "bar6"}]]))))

          (t/is (= nil (.getLatestCompletedBlockTx live-index)))
          ...
          (t/is (= 2097 (:tx-id (.getLatestCompletedBlockTx live-index))))

          (t/is (= 6796 (:tx-id (xt/execute-tx node [[:put-docs :bar {:xt/id "bar7"}]
          ...
          (t/is (= 6796 (:tx-id (.getLatestCompletedBlockTx live-index)))))))))
