...
(t/use-fixtures :each
  (tu/with-opts {:default-tz #xt.time/zone "Europe/London"})
...
(t/deftest can-specify-default-tz-in-query-396
  (let [q (str "SELECT CAST(DATE '2020-08-01' AS TIMESTAMP WITH TIME ZONE) AS tstz "
    ...
    (t/is (= [{:tstz #xt.time/zoned-date-time "2020-08-01T00:00+01:00[Europe/London]"}]
             (xt/q tu/*node* q {})))

    (t/is (= [{:tstz #xt.time/zoned-date-time "2020-08-01T00:00-07:00[America/Los_Angeles]"}]
             (xt/q tu/*node* q {:default-tz #xt.time/zone "America/Los_Angeles"})))))
...
(t/deftest can-specify-default-tz-in-dml-396
  (let [q "INSERT INTO foo (_id, dt, tstz) VALUES (?, DATE '2020-08-01', CAST(DATE '2020-08-01' AS TIMESTAMP WITH TIME ZONE))"]
    ...
    (let [tx (xt/submit-tx tu/*node* [[:sql q ["bar"]]]
                           {:default-tz #xt.time/zone "America/Los_Angeles"})
          q "SELECT foo._id, foo.dt, CAST(foo.dt AS TIMESTAMP WITH TIME ZONE) cast_tstz, foo.tstz FROM foo"]

      (t/is (= #{{:xt/id "foo", :dt #xt.time/date "2020-08-01",
                  :cast-tstz #xt.time/zoned-date-time "2020-08-01T00:00+01:00[Europe/London]"
                  :tstz #xt.time/zoned-date-time "2020-08-01T00:00+01:00[Europe/London]"}
                 {:xt/id "bar", :dt #xt.time/date "2020-08-01"
                  :cast-tstz #xt.time/zoned-date-time "2020-08-01T00:00+01:00[Europe/London]"
                  :tstz #xt.time/zoned-date-time "2020-08-01T00:00-07:00[America/Los_Angeles]"}}

               (set (xt/q tu/*node* q {:basis {:at-tx tx}}))))

      (t/is (= #{{:xt/id "foo", :dt #xt.time/date "2020-08-01",
                  :cast-tstz #xt.time/zoned-date-time "2020-08-01T00:00-07:00[America/Los_Angeles]"
                  :tstz #xt.time/zoned-date-time "2020-08-01T00:00+01:00[Europe/London]"}
                 {:xt/id "bar", :dt #xt.time/date "2020-08-01"
                  :cast-tstz #xt.time/zoned-date-time "2020-08-01T00:00-07:00[America/Los_Angeles]"
                  :tstz #xt.time/zoned-date-time "2020-08-01T00:00-07:00[America/Los_Angeles]"}}
