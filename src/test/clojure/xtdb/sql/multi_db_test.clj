...
(t/deftest dodgy-yaml
  (with-open [node (xtn/start-node)]
    (t/is (= {:sql-state "08P01"
              :message "Invalid database config in `ATTACH DATABASE`: Unknown property 'somethingElse'. Known properties are: critical, externalLog, log, mode, storage" ,
              :detail #xt/error [:incorrect :xtdb/invalid-database-config
                                 "Invalid database config in `ATTACH DATABASE`: Unknown property 'somethingElse'. Known properties are: critical, externalLog, log, mode, storage"
