    ...
    (psql-session
     (fn [send read]
       ...
       (testing "error during query execution"
         ...
         (with-redefs [pgwire/cmd-exec-query (fn [& _]  (throw (Exception. "unexpected")))]
           (send "select 1;\n")
           (is (= ["ERROR:  unexpected"
                   "DETAIL:  {\"category\":\"cognitect.anomalies\\/fault\",\"code\":\"xtdb.error\\/unknown\",\"message\":\"unexpected\"}"]
                  (read :stderr))))

         (with-redefs [pgwire/cmd-exec-query (fn [& _]  (throw (ex-info "with pg code" {::pgwire/severity :error, ::pgwire/error-code "XXXXX"})))]
           (send "select 1;\n")
