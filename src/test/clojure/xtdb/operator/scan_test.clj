...
(t/deftest test-simple-scan
  (with-open [node (xtn/start-node {})]
    ...
    (t/is (= #{{:xt/id :bar, :col1 "bar1", :col2 "bar2"}
               {:xt/id :foo, :col2 "baz2"}}
             (set (tu/query-ra '[:scan {:table xt_docs} [_id col1 col2]]
                               {:node node}))))))
...
(t/deftest test-simple-scan-with-namespaced-attributes
  (with-open [node (xtn/start-node {})]
    ...
    (t/is (= #{{:xt/id :bar, :the-ns/col1 "bar1", :col2 "bar2"}
               {:xt/id :foo}}
             (set (tu/query-ra '[:scan {:table xt_docs} [_id the_ns$col1 col2]]
                               {:node node}))))))
...
(t/deftest test-duplicates-in-scan-1
  (with-open [node (xtn/start-node {})]
    ...
    (t/is (= [{:xt/id :foo}]
             (tu/query-ra '[:scan {:table xt_docs} [_id _id]]
                          {:node node})))))
...
(t/deftest test-chunk-boundary
  (with-open [node (xtn/start-node {:indexer {:rows-per-chunk 20}})]
    ...
    (t/is (= (set (for [i (range 110)] {:xt/id i}))
             (set (tu/query-ra '[:scan {:table xt_docs} [_id]]
                               {:node node}))))))
...
(t/deftest test-metadata
  (with-open [node (xtn/start-node {:indexer {:rows-per-chunk 20}})]
    ...
    (t/is (= (set (concat (for [i (range 20)] {:xt/id i}) (for [i (range 80 100)] {:xt/id i})))
             (set (tu/query-ra '[:scan {:table xt_docs} [{_id (or (< _id 20)
                                                                  (>= _id 80))}]]
                               {:node node})))
  ...
  (with-open [node (xtn/start-node {:indexer {:rows-per-chunk 20}})]
    ...
    (t/is (= []
             (tu/query-ra '[:scan {:table xt_docs} [{_id (< _id 20)}]]
                          {:node node}))
...
(t/deftest test-absent-columns
  (with-open [node (xtn/start-node {})]
    ...
    (t/is (= [{:col2 "bar2", :xt/id :bar}]
             (tu/query-ra '[:scan {:table xt_docs} [_id {col2 (= col2 "bar2")}]]
                          {:node node})))

    ;; column not existent at all
    (t/is (= []
             (tu/query-ra
              '[:scan {:table xt_docs} [_id {col-x (= col-x "toto")}]]
              {:node node})))))
      ...
      (let [old-select->iid-byte-buffer scan/selects->iid-byte-buffer]
        (with-redefs [scan/selects->iid-byte-buffer
          ...
          (t/is (= [{:version 2, :xt/id search-uuid}]
                   (tu/query-ra [:scan {:table 'xt_docs} ['version {'_id (list '= '_id search-uuid)}]]
                                {:node node})))

          (t/is (= [{:version 2, :xt/id search-uuid}]
                   (tu/query-ra [:scan {:table 'xt_docs} ['version {'_id (list '=  search-uuid '_id)}]]
                                {:node node})))

          (t/is (= [{:version 2, :xt/id search-uuid}]
                   (tu/query-ra '[:scan {:table xt_docs} [version {_id (= _id ?search-uuid)}]]
  ...
  (with-open [node (xtn/start-node {:indexer {:page-limit 16}})]
    (let [uuids (tu/uuid-seq 40)
      ...
      (t/is (= [{:xt/id search-uuid}]
               (tu/query-ra [:scan '{:table xt_docs} [{'_id (list '= '_id search-uuid)}]]
                            {:node node}))))))
...
(deftest dont-use-fast-path-on-non-literals-2768
  ...
  (t/is (= [#:xt{:id "foo-start"}]
           (tu/query-ra
            '[:scan
              {:table xt_docs} [{_id (= "foo" (substring _id 1 3))}]]
            {:node tu/*node*}))))
      ...
      (let [tx-key (last (for [i (range 6)]
        ...
        (let [query-opts {:node node
          ...
          (t/is (= [{:xt/id 1,
                   ...
                   (tu/query-ra '[:scan {:table docs} [_id _valid_from]] query-opts)))
          ...
          (t/is (= [{:xt/id 1,
                   ...
                   (tu/query-ra '[:scan {:table docs} [_id _valid_from _valid_to]]
          ...
          (t/is (= (repeat 6 {:xt/id 1})
                   (tu/query-ra '[:scan {:table docs :for-valid-time :all-time} [_id]] query-opts)))
