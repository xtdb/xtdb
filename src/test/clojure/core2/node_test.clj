...
VALUES (1, 'Happy 2024!', DATE '2024-01-01'),
    ...
    (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO posts2 (id, text, application_time_start) VALUES (1, 'Happy 2024!', DATE '2024-01-01')"]
...
(t/deftest test-dml-sees-in-tx-docs
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id, v) VALUES ('foo', 0)"]
...
(t/deftest test-delete-without-search-315
  (letfn [(q []
    ...
    (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id) VALUES ('foo')"]])
    ...
    (c2.sql/submit-tx tu/*node* [[:sql "DELETE FROM foo"]]
...
(t/deftest test-update-set-field-from-param-328
  ...
  (c2.sql/submit-tx tu/*node* [[:sql "UPDATE users FOR PORTION OF APP_TIME FROM ? TO ? AS u SET first_name = ? WHERE u.id = ?"
...
(t/deftest test-put-delete-with-implicit-tables-338
  (letfn [(foos []
            (->> (for [[tk tn] {:xt "xt_docs"
                                :t1 "explicit_table1"
                                :t2 "explicit_table2"}]
                   [tk (->> (c2.sql/q tu/*node* (format "SELECT t.id, t.v FROM %s t WHERE t.application_time_end = END_OF_TIME" tn))
                            (into #{} (map :v)))])
                 (into {})))]
...
(t/deftest test-array-element-reference-is-one-based-336
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id, arr) VALUES ('foo', ARRAY[9, 8, 7, 6])"]])
...
(t/deftest test-error-handling-inserting-strings-into-app-time-cols-397
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id, application_time_start) VALUES (1, '2018-01-01')"]])

  ;; TODO check the rollback error when it's available, #401
  (t/is (= [] (c2.sql/q tu/*node* "SELECT foo.id FROM foo"))))
...
(t/deftest test-double-quoted-col-refs
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id, \"kebab-case-col\") VALUES (1, 'kebab-case-value')"]])
...
(t/deftest test-select-left-join-471
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO foo (id, x) VALUES (1, 1), (2, 2)"]
...
(t/deftest test-mutable-data-buffer-bug
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO t1(id) VALUES(1)"]])
...
(t/deftest test-differing-length-lists-441
  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO t1(id, data) VALUES (1, [2, 3])"]
                               [:sql "INSERT INTO t1(id, data) VALUES (2, [5, 6, 7])"]])

  (t/is (= [{:data [2 3]} {:data [5 6 7]}]
           (c2.sql/q tu/*node* "SELECT t1.data FROM t1")))

  (c2.sql/submit-tx tu/*node* [[:sql "INSERT INTO t2(id, data) VALUES (1, [2, 3])"]
