...
sealed class Variance(
    ...
    override fun build(al: BufferAllocator) = object : AggregateSpec {
        private val sumxAgg = Sum(fromName, "sumx", f64Type, hasZeroRow).build(al)
        private val sumx2Agg = Sum("x2", "sumx2", f64Type, hasZeroRow).build(al)
        ...
        override fun aggregate(inRel: RelationReader, groupMapping: GroupMapping) {
            val inVec = inRel.vectorForOrNull(fromName) as? DoubleVector ?: return
            
            val x2Vec = DoubleVector(al, "x2", nullable = true)
            try {
                inVec.squareInto(x2Vec)
