...
(definterface IHistogram
  (^xtdb.temporal.histogram.IHistogram update [^double x])
  (^double sum [^double x])
...
(defn ->histogram ^xtdb.temporal.histogram.Histogram [^long max-bins]
...
(definterface IMultiDimensionalHistogram
  (^xtdb.temporal.histogram.IMultiDimensionalHistogram update [^doubles x])
  (^doubles getMins [])
  (^doubles getMaxs [])
  (^long getTotal [])
  (^java.util.List getBins [])
  (^xtdb.temporal.histogram.IHistogram projectAxis [^int axis]))

(definterface IMultiDimensionalBin
  (^doubles getValue [])
  (^long getCount [])
  (^xtdb.temporal.histogram.IBin projectAxis [^int axis]))
...
(defn ->multidimensional-histogram ^xtdb.temporal.histogram.IMultiDimensionalHistogram [^long max-bins ^long k]
