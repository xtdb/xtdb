(ns xtdb.pgwire
  ...
  (:import [clojure.lang PersistentQueue]
           ...
           (xtdb.antlr SqlVisitor SqlBaseVisitor Sql$ParamExprContext)
        ...
        (try
          (.accept (plan/parse-statement sql-trimmed)
                   (proxy
                     ...
                     (visitDirectSqlStatement [ctx] (.accept (.directlyExecutableStatement ctx) this))
...
(defn parse
  ...
  (let [{:keys [session latest-submitted-tx]} @conn-state
        ...
        {:keys [err statement-type] param-cast-types :param-types :as stmt} (interpret-sql query {:default-tz (.getZone ^Clock (get-in @conn-state [:session :clock]))
