...
(defn- client-err
  ...
  ([client-msg {:keys [error-type] :as data}]
   (ex-info client-msg (assoc data ::client-error
                              (-> (err-protocol-violation client-msg)
                                  (assoc :error-type error-type))))))
...
(defn cmd-send-error
  ...
  (if (#{:dml :transaction} error-type)
    (when tx-error-counter
      (.increment tx-error-counter)) ;; TODO Add tx error counter
...
(defn- execute-tx [{:keys [node]} dml-ops tx-opts]
  (try
    ...
    (catch xtdb.IllegalArgumentException e
      (throw (client-err (ex-message e) {:error-type :transaction})))
    (catch Throwable e
...
(defn cmd-commit [{:keys [conn-state] :as conn}]
  (let [{:keys [transaction session]} @conn-state
    ...
    (if failed
      (throw (client-err "transaction failed" {:error-type :transaction}))
      ...
      (try
        (let [{:keys [tx-id error]} (when (= :read-write access-mode)
          ...
          (when error
            (throw (client-err (ex-message error) {:error-type :transaction}))))
        (catch InterruptedException e (throw e))
        (catch Exception e
