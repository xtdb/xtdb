...
(defmethod xtn/apply-config! :xtdb/garbage-collector [^Xtdb$Config config, _, [enabled blocks-to-keep approx-run-interval]]
  (.garbageCollector config 
                     (cond-> (GarbageCollectorConfig.)
                       enabled (.setEnabled enabled)
                       blocks-to-keep (.setBlocksToKeep blocks-to-keep) 
                       approx-run-interval (.setApproxRunInterval approx-run-interval))))
