...
(defn ->indirect-vec ^xtdb.vector.IIndirectVector [^ValueVector in-vec, ^ints idxs]
...
(defn ->indirect-rel
  (^xtdb.vector.IIndirectRelation [cols]
   (->indirect-rel cols
                   (if-let [^IIndirectVector col (first cols)]
                     (.getValueCount col)
                     0)))

  (^xtdb.vector.IIndirectRelation [cols ^long row-count]
...
(defn <-root ^xtdb.vector.IIndirectRelation [^VectorSchemaRoot root]
...
(defn select ^xtdb.vector.IIndirectRelation [^IIndirectRelation in-rel, ^ints idxs]
  (->indirect-rel (for [^IIndirectVector in-col in-rel]
                    (.select in-col idxs))
                  (alength idxs)))

(defn copy ^xtdb.vector.IIndirectRelation [^IIndirectRelation in-rel, ^BufferAllocator allocator]
  (->indirect-rel (for [^IIndirectVector in-col in-rel]
