...
(defn- surround-parser-with-ws ^xtdb.sql.parser.Parser$AParser [parser ^Pattern ws-pattern]
...
(defn build-ebnf-parser
  ...
  ([^Map grammar ws-pattern ->raw?]
   (let [^Map rule->id (zipmap (keys grammar) (range))
         ^"[Lxtdb.sql.parser.Parser$AParser;" rules (make-array Parser$AParser (count rule->id))]
               ...
               (let [parser (case tag
                              :nt (let [body (get grammar (:keyword parser))]
                                    (if (and (or hide (= {:reduction-type :raw} (:red body)))
                                      ...
                                      (Parser$NonTerminalParser. (or (get rule->id (:keyword parser))
                                                                     (throw (UnsupportedOperationException. (pr-str (:keyword parser))))))))
...
(comment
  ...
  (= (sql-parser "SELECT pk FROM tab0 WHERE ((col0 >= 6) OR (col1 = 2.11 AND col3 BETWEEN 3 AND 4) AND ((col1 > 9.58 AND col0 <= 4)) AND (col0 >= 6))" :directly_executable_statement)
     (sql-parser (xtdb.sql.logic-test.xtdb-engine/remove-unnecessary-parens "SELECT pk FROM tab0 WHERE ((col0 >= 6) OR (col1 = 2.11 AND col3 BETWEEN 3 AND 4) AND ((col1 > 9.58 AND col0 <= 4)) AND (col0 >= 6))") :directly_executable_statement))
