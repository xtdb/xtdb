...
(defn- sum-out-type ^org.apache.arrow.vector.types.pojo.ArrowType [^VectorType from-type]
  (or (LeastUpperBound/of (map VectorType/.getArrowType (.getUnionLegs from-type)))
      (throw (err/incorrect :xtdb.group-by/cannot-sum-type
                            (str "Cannot compute SUM over type " from-type)
                            {:type from-type}))))

(defmethod ->aggregate-factory :sum [{:keys [from-name from-field to-name zero-row?]}]
  (let [from-type (types/->type (or from-field :null))]
    (Sum. (str from-name) (str to-name) (sum-out-type from-type) zero-row?)))

(defmethod ->aggregate-factory :avg [{:keys [from-name to-name zero-row?]}]
  (Average. (str from-name) (str to-name) zero-row?))
