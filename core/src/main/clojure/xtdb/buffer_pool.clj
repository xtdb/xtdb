...
(defmethod xtn/apply-config! ::in-memory [^Xtdb$Config config _ _]
  (.storage config (Storage/inMemory)))

(defmethod xtn/apply-config! ::local [^Xtdb$Config config _ {:keys [path]}]
  (.storage config (Storage/local (util/->path path))))
...
(defmethod xtn/apply-config! ::remote [^Xtdb$Config config _ {:keys [object-store]}]
  (.storage config (Storage/remote (let [[tag opts] object-store]
