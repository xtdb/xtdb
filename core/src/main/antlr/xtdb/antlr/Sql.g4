...
// every keyword token except clause-starters (explicitAsKeyword) and those already in identifier
aliasKeyword
    ...
    | 'EXCLUDE' | 'EXECUTE' | 'EXISTS' | 'EXPLAIN' | 'EXTRACT'
    | 'FALSE' | 'FIRST_VALUE' | 'FIRST' | 'FLAG' | 'FLOAT' | 'FOLLOWING' | 'FRAME_ROW' | 'FULL'
    | 'GENERATE_SERIES' | 'GROUPS'
    | 'HAS_ANY_COLUMN_PRIVILEGE' | 'HAS_SCHEMA_PRIVILEGE' | 'HAS_TABLE_PRIVILEGE'
    | 'HOUR'
    | 'IGNORE' | 'IMMEDIATELY' | 'IN' | 'INNER' | 'INT' | 'INTEGER' | 'INTERVAL'
    | 'IS' | 'ISODOW' | 'ISOLATION'
    | 'JOIN'
    | 'KEYWORD'
    | 'LAG' | 'LAGS' | 'LAST_VALUE' | 'LAST' | 'LATERAL' | 'LEAD' | 'LEADING' | 'LEADS'
    | 'LEFT' | 'LEVEL' | 'LIKE_REGEX' | 'LIKE' | 'LOCAL' | LOCAL_DATE | LOCAL_TIME | LOCAL_TIMESTAMP
    ...
    | 'OBJECT' | 'OCTETS' | 'OF' | OFF | 'ON' | 'ONLY' | 'OR' | 'ORDINALITY' | 'OTHERS'
