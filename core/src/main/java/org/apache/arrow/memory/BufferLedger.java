...
public class BufferLedger implements ValueWithKeyIncluded<BufferAllocator>, ReferenceManager {
  ...
  public void retain(int increment) {
    ...
    while(true) {
        final int currentRefCount = bufRefCnt.get();
        Preconditions.checkArgument(currentRefCount > 0);
        if (bufRefCnt.compareAndSet(currentRefCount, currentRefCount + increment)) {
            if (historicalLog != null) {
