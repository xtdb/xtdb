...
class LogProcessorSimTest : SimulationTestBase() {
                            ...
                            launch(dispatcher) {
                                ...
                                launch {
                                    repeat(srcLogEventCount) {
                                        ...
                                        listOf(leader, followerA, followerB).requestAsserts()
                                    }
                                    simExtSource.awaitQuiescence()
                                    replicaLog.awaitAllDelivered()
                        ...
                        launch(dispatcher) {
                            ...
                            launch {
                                repeat(srcLogEventCount) {
                                    ...
                                    if (rand.nextInt(100) < 30) listOf(nodeA, nodeB).requestAsserts()
                                }
                                simExtSource.awaitQuiescence()
                                replicaLog.awaitAllDelivered()
     ...
     * folds, so what this puts under the invariants below is a leader's proof of life arriving in the middle
     * of a block cycle and a leadership change, and a superseded leader still asserting behind the claim that
     * beat it.
     */
                ...
                SimNode("test-db", bp, indexerConfig, simExtSource).use { incumbent ->
                    SimNode("test-db", bp, indexerConfig, simExtSource).use { refusedA ->
                        SimNode("test-db", bp, indexerConfig, simExtSource).use { refusedB ->
                            ...
                            // The point of the whole case: a node that treated the refusal as its own fault
                            // would stop answering queries over a write it never had to make.
                            assertNoNodeFailed(nodes)
                            assertNodesConverged(nodes)
                            refused.forEach { assertSnapshotHasNoAbortedRows(it) }
                        }
                    }
                }
            }
