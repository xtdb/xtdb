...
class ColumnMetaTest {
    ...
    @Test
    fun `a node takes its name from the key it is held under, at every depth`() {
        val col = ColumnMeta.fromProto(
            "s", ColumnMeta.of("s", VectorType.Struct(mapOf("z" to VectorType.I64))).toProto()
        )
