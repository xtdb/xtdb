...
temporal semantics] under the hood. Advanced temporal queries should
be possible, but not necessarily highly optimised. Time-slice queries
must still be competitive.

Internally, we will base the temporal data model on the
https://www2.cs.arizona.edu/~rts/pubs/ISDec94.pdf[Bitemporal
Conceptual Data Model] from Jensen, Soo and Snodgrass.
