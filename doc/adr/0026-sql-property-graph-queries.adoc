...
Supporting SQL/PGQ we also avoid the need to drop into Datalog for
many cases, making SQL more competitive when querying graphs.

SQL/PGQ is a read-only subset of the upcoming
https://www.gqlstandards.org/[GQL standard] embedded in SQL via
the GRAPH_TABLE construct. It looks somewhat like this:

SELECT g.* FROM GRAPH_TABLE ( myGraph,
    MATCH
...
How to interpret the edges properly is open for analysis. Something
like IS Created (can also be written as :Created) could be seen as the
