...
To motivate this, if we pull apart the problem statement we see that there are a few dimensions to consider, all with their own challenges.
...
One intrinsic issue is the friction caused by our constraints around asynchronicity and atomicity being in conflict with typical expectations of a SQL database (such as what is imposed upon our postgres server). It might be wise to buy as much time as possible to improve our chances of a successful and clear fitting of core2's semantics to our postgres server. (If pgwire sticks around!)
