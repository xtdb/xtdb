...
We need support for temporal queries and a natural way to access nested data in documents.
...
. Leverage existing parser, like https://partiql.org/[PartiQL] or https://github.com/JSQLParser/JSqlParser[JSQLParser].
...
. Structured user-defined types (nested access) and booleans as of SQL-1999.
. Take inspiration from PartiQL.

XTDB should not require a schema to work, so columns in SQL will need to qualified.
