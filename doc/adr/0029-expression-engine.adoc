...
The expression engine needs to support the full Arrow type system.
...
The engine needs to be extensible and able to support the basic expected functions in SQL.
This is today possible through a multi-method.

=== Group by

The group-by operator in the logical plan needs to use the same semantics as the operators in the expression engine for aggregation.
Currently aggregation is handled by a different system.

=== Order by

The order-by operator in the logical plan needs to behave the same way as the comparison operators of the expression engine.
...
All overhead should move to per-batch and primitive operators and stack allocated locals should be used in the inner loop.
...
The easiest is to have both Datalog and SQL share the low-level semantics, for reasons of both technical and clarity.
That said, it would be possible to make the expression engine have different modes when it comes to NULL.
