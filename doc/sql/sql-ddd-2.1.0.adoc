...
This document is written for a fictitious `2.1.0` version of XTDB.
It assumes the transaction model is not unlike the current transaction model for XTDB 1.17.0.
...
This is probably not what we want, long-term, but a lot of this document could reasonably be implemented on top of XTDB 1.x.
