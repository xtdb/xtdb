...
The place this is most likely to cause confusion is between `SYSTEM_TIME` / `VALID_TIME` (which are reserved words) and `xt$system_from` / `xt$system_to` / `xt$valid_from` / `xt$valid_to`, which are built-in column names.
