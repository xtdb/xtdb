...
1. monotonically increasing clocks within a session, across reads and writes.
2. a single source of time derived from LogAppendTime -> SYSTEM_TIME / CURRENT_TIMESTAMP -> APPLICATION_TIME (unless provided as constraints).
